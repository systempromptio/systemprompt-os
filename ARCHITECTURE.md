...
Define clear interfaces that allow users to plug in their own implementations rather than forcing specific solutions.
...
│       │   └── tools/        # Core tools
│       │       ├── config_agent.ts    # Configure agent settings
│       │       ├── setup_provider.ts  # Setup memory/action/auth providers
│       │       ├── list_providers.ts   # List available providers
│       │       └── system_info.ts     # Get system information
...
│   │   ├── memory-redis/      # Redis memory provider (plugin)
│   │   │   ├── module.yaml    # type: plugin, provides: memory
│   │   │   ├── index.ts       # Redis implementation
│   │   │   └── README.md      # Module documentation
...
│   │   ├── daemon.ts          # Daemon module interface
│   │   ├── service.ts         # Service module interface
│   │   ├── plugin.ts          # Plugin module interface
│   │   ├── cli.ts             # CLI command interface
│   │   └── registry.ts        # Module registry interface
│   ├── module-loader/          # Module loading and management
│   │   ├── loader.ts          # Module loader implementation
...
│       └── src/               # Source code
│           ├── core/          # Core CLI commands (always available)
...
The registry (`server/mcp/registry.ts`) manages both types:
