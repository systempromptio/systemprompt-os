...
import type { AuthAuditEntry, AuthAuditAction, AuditRow } from '../types/index.js';
...
export class AuthAuditService {
   ...
   * Record logout
   */
  async recordLogout(userId: string, ipAddress?: string, userAgent?: string): Promise<void> {
    await this.recordAudit({
      userId,
      action: 'auth.logout',
      ...(ipAddress && { ipAddress }),
  ...
  async cleanupOldEntries(): Promise<number> {
    try {
      ...
      if (count > 0) {
        ...
        this.logger.info('Cleaned up old audit entries', {
          count,
