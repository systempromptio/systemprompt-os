...
export class TokenService {
  ...
  async createToken(input: TokenCreateInput): Promise<AuthToken> {
    try {
      ...
      this.logger.error('Failed to create token', { input, error });
  ...
  async createJWT(
    ...
    try {
      ...
      this.logger.error('Failed to create JWT', { userId, error });
  ...
  async revokeToken(tokenId: string): Promise<void> {
    try {
      ...
      this.logger.error('Failed to revoke token', { tokenId, error });
  ...
  async revokeUserTokens(userId: string, type?: string): Promise<void> {
    try {
      ...
      this.logger.info('User tokens revoked', { userId, type });
    } catch (error) {
  ...
  async listUserTokens(userId: string): Promise<AuthToken[]> {
    try {
      ...
      return result.map((row): AuthToken => ({
        id: row.id,
        userId: row.user_id,
      ...
      this.logger.error('Failed to list user tokens', { userId, error });
