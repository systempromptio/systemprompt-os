/**
 * @fileoverview Validate data integrity command
...
export function createValidateCommand(db: DatabaseService, logger: Logger): Command {
      ...
      try {
        ...
        if (options.checkOrphans) {
          for (const table of tables) {
            ...
            orphans.forEach((orphan) => issues.push(orphan));
          }
        }
        ...
        if (options.checkIndexes) {
          ...
          indexIssues.forEach((issue) => issues.push(issue));
        }

        // Check for common data issues
        for (const table of tables) {
          const dataIssues = await checkDataIntegrity(db, table);
          dataIssues.forEach((issue) => issues.push(issue));
        }
        ...
        process.exit(
          issues.some((i) => i.severity === 'critical' || i.severity === 'error') ? 1 : 0,
        );
      } catch (error) {
...
async function checkOrphans(db: DatabaseService, table: string): Promise<ValidationIssue[]> {
  ...
  const orphanChecks: { [key: string]: { parent: string; fk: string; pk: string } } = {
    auth_user_roles: { parent: 'auth_users', fk: 'user_id', pk: 'id' },
    auth_tokens: { parent: 'auth_users', fk: 'user_id', pk: 'id' },
    auth_sessions: { parent: 'auth_users', fk: 'user_id', pk: 'id' },
    permissions_user_permissions: { parent: 'auth_users', fk: 'user_id', pk: 'id' },
    permissions_role_permissions: { parent: 'permissions_roles', fk: 'role_id', pk: 'id' },
  };
...
async function checkIndexes(db: DatabaseService): Promise<ValidationIssue[]> {
  ...
  const recommendedIndexes = [
    ...
    { table: 'auth_tokens', column: 'token_hash' },
    { table: 'auth_sessions', column: 'session_id' },
    { table: 'mcp_stats', column: 'created_at' },
  ];

  for (const rec of recommendedIndexes) {
    const hasIndex = indexes.some(
      (idx) => idx.tbl_name === rec.table && idx.name.toLowerCase().includes(rec.column),
    );
...
async function checkDataIntegrity(db: DatabaseService, table: string): Promise<ValidationIssue[]> {
  ...
  try {
    ...
    for (const col of columns.filter((c) => c.notnull === 1)) {
...
async function fixIssues(
  ...
  try {
    for (const issue of issues.filter((i) => i.fixable && i.fixQuery)) {
      try {
