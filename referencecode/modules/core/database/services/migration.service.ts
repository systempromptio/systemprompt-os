...
export class MigrationService {
  ...
  static initialize(databaseService: DatabaseService, logger?: ILogger): MigrationService {
    ...
    MigrationService.instance.logger = logger || MigrationService.instance.logger;
  ...
  async runMigrations(): Promise<void> {
    ...
    const pending = this.migrations.filter((m) => !applied.has(this.getMigrationKey(m)));
  ...
  async getPendingMigrations(): Promise<Migration[]> {
    ...
    return this.migrations
      .filter((m) => !applied.has(this.getMigrationKey(m)))
      .map((m) => ({
        ...m,
        name: m.filename,
      }));
