...
import type { Logger, LoggerConfig, LogLevel, LogLevelName } from '@/modules/core/logger/types/index.js';
import {
  LoggerError,
...
export class LoggerService implements Logger {
  ...
  async initialize(config: LoggerConfig): Promise<void> {
    ...
    try {
      ...
      throw new LoggerInitializationError('Failed to initialize logger', error as Error);
    }
  }
  ...
  async clearLogs(logFile?: string): Promise<void> {
    ...
    try {
      ...
      throw new LoggerFileWriteError(logFile || 'all log files', error as Error);
    }
  }
  ...
  async getLogs(logFile?: string): Promise<string[]> {
    ...
    try {
      ...
      throw new LoggerFileReadError(logFile || 'log files', error as Error);
    }
  }
