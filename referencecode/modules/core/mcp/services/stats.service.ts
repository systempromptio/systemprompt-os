...
export class MCPStatsService {
  ...
  private readonly inMemoryStats = {
    tools: {
   ...
   */
  async recordToolExecution(toolName: string, success: boolean, durationMs: number): Promise<void> {
  ...
  /**
   * Record a prompt creation
   */
  async recordPromptCreation(promptName: string): Promise<void> {
    try {
      await this.db.execute(
        `INSERT INTO mcp_stats (type, name, operation, success, created_at)
         VALUES (?, ?, ?, ?, ?)`,
        ['prompt', promptName, 'create', 1, new Date().toISOString()],
      );
    } catch (error) {
      this.logger.error('Failed to record prompt creation', error);
    }
  }

  /**
   * Record a prompt update
   */
  async recordPromptUpdate(promptName: string): Promise<void> {
    try {
      await this.db.execute(
        `INSERT INTO mcp_stats (type, name, operation, success, created_at)
         VALUES (?, ?, ?, ?, ?)`,
        ['prompt', promptName, 'update', 1, new Date().toISOString()],
      );
    } catch (error) {
      this.logger.error('Failed to record prompt update', error);
    }
  }

  /**
   * Record a prompt deletion
   */
  async recordPromptDeletion(promptName: string): Promise<void> {
    try {
      await this.db.execute(
        `INSERT INTO mcp_stats (type, name, operation, success, created_at)
         VALUES (?, ?, ?, ?, ?)`,
        ['prompt', promptName, 'delete', 1, new Date().toISOString()],
      );
    } catch (error) {
      this.logger.error('Failed to record prompt deletion', error);
    }
  }
