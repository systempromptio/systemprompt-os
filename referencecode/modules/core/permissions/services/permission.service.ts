...
export class PermissionService {
  constructor(
    ...
    private readonly defaultPermissions: Record<string, Array<{ resource: string; actions: string[] }>>,
  ...
  async grantPermission(input: PermissionGrantInput): Promise<void> {
    try {
      ...
      if (input.conditions) {
        details['conditions'] = input.conditions;
      }
      if (input.expiresAt) {
  ...
  async revokePermission(
    ...
    try {
      ...
      // Audit
      const revokeDetails: Record<string, any> = {};
      if (scope) {
