  ...
  async execute(context: CLIContext): Promise<void> {
    try {
      ...
      if (format === 'json') {
        ...
        console.log(
          `  Memory:       ${formatBytes(status.memory.used)} / ${formatBytes(status.memory.total)} (${status.memory.usagePercent.toFixed(1)}%)`,
        );
        console.log(
          `  Disk:         ${formatBytes(status.disk.used)} / ${formatBytes(status.disk.total)} (${status.disk.usagePercent.toFixed(1)}%)`,
        );
        ...
        if (detailed) {
          ...
          console.log(
            `  Load Average: ${status.cpu.loadAverage.map((l) => l.toFixed(2)).join(', ')}`,
          );
