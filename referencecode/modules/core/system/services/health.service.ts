...
export class HealthService {
  private readonly checks: Map<string, () => Promise<HealthCheck>>;
  ...
  private registerDefaultChecks(): void {
    ...
    if (enabledChecks.includes('memory')) {
      this.checks.set('memory', async () => this.checkMemory());
    }

    if (enabledChecks.includes('cpu')) {
      this.checks.set('cpu', async () => this.checkCPU());
    }

    if (enabledChecks.includes('disk')) {
      this.checks.set('disk', async () => this.checkDisk());
    }

    if (enabledChecks.includes('modules')) {
      this.checks.set('modules', async () => this.checkModules());
    }
  }
  ...
  private async checkDisk(): Promise<HealthCheck> {
    ...
    try {
      ...
      import('fs').writeFileSync(testFile, 'test');
      import('fs').unlinkSync(testFile);
