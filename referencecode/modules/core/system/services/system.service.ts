...
export class SystemService {
  ...
  private parseUnixDiskInfo(output: string): DiskInfo {
    ...
    const total = (parseInt(data[1]) || 0) * 1024; // Convert from KB to bytes
    const used = (parseInt(data[2]) || 0) * 1024;
    const free = (parseInt(data[3]) || 0) * 1024;
    const usagePercent = parseInt(data[4]) || 0;
