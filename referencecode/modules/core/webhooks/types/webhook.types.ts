...
export interface IWebhookService {
  createWebhook(data: CreateWebhookDto): Promise<WebhookConfig>;
  getWebhook(id: string): Promise<WebhookConfig | null>;
  listWebhooks(filter?: WebhookFilter): Promise<WebhookConfig[]>;
  updateWebhook(id: string, data: UpdateWebhookDto): Promise<WebhookConfig | null>;
  deleteWebhook(id: string): Promise<boolean>;
  triggerWebhook(event: WebhookEvent, data: any, metadata?: Record<string, any>): Promise<void>;
  testWebhook(id: string): Promise<WebhookTestResult>;
  getWebhookStats(id: string): Promise<WebhookStats | null>;
}

import { Token } from 'typedi';

/**
 * Dependency injection token for Webhook service
 */
export const WEBHOOK_TOKEN = new Token<IWebhookService>('core.webhook');