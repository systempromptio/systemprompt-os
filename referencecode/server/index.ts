...
export async function createApp(): Promise<express.Application> {
  ...
  // Load remaining modules (core modules already bootstrapped)
  const moduleLoader = getModuleLoader();
  await moduleLoader.loadModules();

  // Auth module should already be started by bootstrap, but ensure providers are initialized
  const authModule = moduleLoader.getModule('auth');
  if (authModule && typeof authModule.start === 'function' && !authModule.initialized) {
...
export async function startServer(
  ...
  const server = app.listen(serverPort, '0.0.0.0', async () => {
    ...
    logger.info(
      `🔐 OAuth2 discovery: http://localhost:${serverPort}/.well-known/oauth-protected-resource`,
    );
