...
import { {Module}ModuleExportsSchema } from './types/permissions.module.generated';

// Module MUST validate its structure
const moduleSchema = createModuleSchema({Module}ModuleExportsSchema);
...
```typescript
export class {Module}Module extends BaseModule<I{Module}ModuleExports> {
...
1. **Database Types & Schemas**
   ...
   export interface I{Module}Row { ... }
   export const {Module}RowSchema = z.object({ ... });
   
   // Enums with validation
   export enum {Module}Status { ... }
   export const {Module}StatusSchema = z.nativeEnum({Module}Status);
   ```

2. **Domain Types & Schemas**
   ```typescript
   // Create/Update data schemas
   export const {Module}CreateDataSchema = z.object({ ... });
   export const {Module}UpdateDataSchema = z.object({ ... }).partial();
   
   // Type inference
   export type I{Module}CreateData = z.infer<typeof {Module}CreateDataSchema>;
   ```

3. **Module Export Schemas**
   ```typescript
   // Validates module's export interface
   export const {Module}ModuleExportsSchema = z.object({
...
./bin/systemprompt dev generate-types {module-name}
```
...
1. **Service Methods**
   ...
   // Validate inputs
   async create(data: unknown): Promise<I{Module}> {
     const validated = {Module}CreateDataSchema.parse(data);
     const result = await this.repository.create(validated);
     return {Module}RowSchema.parse(result); // Validate output
   }
   ```
...
2. **Repository Methods**
   ...
   async findById(id: string): Promise<I{Module} | null> {
     ...
     return result ? {Module}RowSchema.parse(result) : null;
   }
   ```
...
4. **Module Exports**
   ...
   get exports(): I{Module}ModuleExports {
     const exports = {
     ...
     return {Module}ModuleExportsSchema.parse(exports);
   }
   ```
...
export class {Module}Service {
  ...
  async create(data: unknown): Promise<I{Module}> {
    ...
    // Validate output
    return {Module}RowSchema.parse(result);
  }
  
  async update(id: string, data: unknown): Promise<I{Module}> {
    // Validate partial update data
    const validated = {Module}UpdateDataSchema.parse(data);
    
    const result = await this.repository.update(id, validated);
    return {Module}RowSchema.parse(result);
  }
  
  async list(filters: unknown): Promise<I{Module}[]> {
    // Define and validate filter schema
    const filterSchema = z.object({
      status: {Module}StatusSchema.optional(),
    ...
    return z.array({Module}RowSchema).parse(results);
  }
}
```
...
import { {Module}RowSchema } from '../types/database.generated';

export class {Module}Repository {
  async create(data: I{Module}CreateData): Promise<I{Module}> {
    const result = await this.db.query(
    ...
    return {Module}RowSchema.parse(result.rows[0]);
  }
