...
./bin/systemprompt agents {command}
...
- **MUST** follow the exact pattern:
  ...
  export const agentsCommands = {
    name: 'agents',
    alias: '{shortname}',  
    description: 'Agents module commands',
...
export const command: ICLICommand = {
  ...
  execute: async (context: ICLIContext): Promise<void> => {
    ...
    try {
      const service = AgentsService.getInstance();
      ...
      logger.error(LogSource.AGENTS, 'Command failed', { error });
      process.exit(1);
    }
  },
};
...
2. **Command Implementation**: Each command is implemented in `src/modules/core/agents/cli/{command}.ts`
3. **Database Storage**: During bootstrap, commands are registered in the `cli_commands` table with:
   - `command_path`: Full path like `agents:command` (e.g., `agents:create`)
   - `executor_path`: Path to the TypeScript file implementing the command
   - Command metadata (description, options, etc.)
4. **CLI Execution**: When running `systemprompt agents {command}`:
   - The CLI main.ts loads commands from the database
   - Parses the command path (`agents:{command}`) to find the executor
   - Dynamically imports the command module
   - Executes the command with parsed context

### Command Handler Resolution
- The handler string in `cli/index.ts` (e.g., `handler: 'agents:{command}'`) maps to the command path
...
- `agents.module.generated.ts` - Create/Update data schemas
- `agents.service.generated.ts` - Service method schemas
...
```typescript
import { UserUpdateDataSchema } from '../types/agents.module.generated';
...
```typescript
// In agents/utils/cli-validation.ts
import { z } from 'zod';
import { CliOutputService } from '@/modules/core/cli/services/cli-output.service';
import { 
  UserCreateDataSchema,
...
3. **Type Generation**:
   - Run `./bin/systemprompt dev generate-types agents` after schema changes
