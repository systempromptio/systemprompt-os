...
import { z } from 'zod';
import { AgentsStatusSchema, AgentsStatus, AgentsTypeSchema } from '../types/database.generated';
...
// Create command schema - extends autogenerated schema with CLI-specific fields and defaults
export const createCommandArgsSchema = z.object({
  // Required fields from CLI args
  name: z.string().min(1, 'Agent name is required'),
  description: z.string().min(1, 'Agent description is required'),
  instructions: z.string().min(1, 'Agent instructions are required'),
  type: AgentsTypeSchema,
  // Fields with defaults that match database
  status: AgentsStatusSchema.default(AgentsStatus.STOPPED),
  assigned_tasks: z.number().default(0),
...
// Update command schema - similar to create but with optional fields
export const updateCommandArgsSchema = z.object({
  id: z.string().min(1, 'Agent ID is required'),
  format: z.enum(['text', 'json']).default('text'),
  // Optional update fields
  name: z.string().min(1, 'Agent name is required').optional(),
  description: z.string().min(1, 'Agent description is required').optional(),
