...
### agents.service.ts (MANDATORY)
**MUST** be named exactly as the module name (e.g., `agents.service.ts` for agents module). This is the primary service that implements the main module interface.
**MUST** be located directly in the services directory (e.g., `/services/agents.service.ts`), NOT inside a subfolder.
...
### Main Module Service
The primary service (e.g., `agents.service.ts`) serves as:
...
```
services/
├── agents.service.ts           # Main module service (MANDATORY - at root level)
├── validation.service.ts         # Additional service (at root level)
├── agents/                     # Optional support folder for agents.service.ts
│   ├── helpers.ts
│   ├── validators.ts
│   └── types.ts
...
export class AgentsService {
  ...
  async createAgent(data: IAgentCreateData): Promise<IAgent> {
    ...
    return await this.repository.createAgent(encryptedData);
  }
}
```
...
export class AgentsService implements IAgentsService {
  ...
  async initialize(): Promise<void> {
    ...
    this.logger?.info(LogSource.AGENTS, 'AgentsService initialized');
  }
...
- **MUST** implement the auto-generated service interface from `types/agents.service.generated.ts` (main service only)
...
- **MUST** use `AgentsRepository.getInstance()` for data access (main service only)
...
export class AgentsService {
  ...
  async createAgent(data: IAgentCreateData, userId: string): Promise<IAgent> {
    ...
    this.eventBus.on<UserDataRequestEvent>(
      ...
      async (event) => {
        try {
          ...
          this.logger?.error(LogSource.AGENTS, 'Failed to handle user data request', { error });
        }
      }
    );
  }
...
export class AgentsService {
  ...
  private constructor() {
    ...
    try {
      ...
      this.logger?.warn(LogSource.AGENTS, 'Audit service not available');
    }
  }
...
async createAgent(data: IAgentCreateData): Promise<IAgent> {
  ...
  this.logger?.info(LogSource.AGENTS, `Creating agent: ${data.name}`);
  ...
  this.eventBus.emit(AgentsEvents.{ENTITY}_CREATED, {
    agentId: agent.id,
    timestamp: now
  });
...
### Validation Methods
```typescript
private async validateAgentData(data: IAgentCreateData): Promise<void> {
  ...
  if (existing) {
    throw new Error(`Agent name already exists: ${data.name}`);
  }
...
```typescript
private setupEventHandlers(): void {
  this.eventBus.on<AgentDataRequestEvent>(
    AgentsEvents.{ENTITY}_DATA_REQUEST, 
    async (data: unknown) => {
      try {
        const event = data as AgentDataRequestEvent;
        const agent = await this.getAgent(event.agentId);
        
        const response: AgentDataResponseEvent = {
          requestId: event.requestId,
          agent: agent ? { /* mapped data */ } : null
        };
        
        this.eventBus.emit(AgentsEvents.{ENTITY}_DATA_RESPONSE, response);
      } catch (error) {
        this.logger?.error(LogSource.AGENTS, 'Event handler error', { error });
...
### Generated Types
- **MUST** use auto-generated interfaces from `types/agents.module.generated.ts`
...
### Return Types
- **MUST** return domain interfaces (e.g., `IAgent`), not database rows
...
if (data.status === 'inactive' && hasActiveRelations) {
  throw new Error('Cannot deactivate agent with active relations');
}
...
```typescript
// Operation start
this.logger?.info(LogSource.AGENTS, `Starting operation: ${operation}`);

// Success
this.logger?.info(LogSource.AGENTS, `Operation completed: ${operation}`);

// Errors
this.logger?.error(LogSource.AGENTS, 'Operation failed', {
...
### Log Sources
- **MUST** use module-specific LogSource (e.g., `LogSource.AGENTS`)
