      ...
      execute: async (context: CLIContext): Promise<void> => {
        ...
        try {
          ...
          const user = await db
            ...
            .then((rows: { id: string; email: string }[]) => { return rows[0] });
          ...
          const role = await db
            ...
            .then((rows: { id: string; name: string }[]) => { return rows[0] });
          ...
          const existing = await db
            ...
            .then((rows: unknown[]) => { return rows.length > 0 });
        ...
        try {
          ...
          if (args['role'] === 'admin') {
            const adminCount = await db
              ...
              .then((rows: { count: number }[]) => { return rows[0]?.count || 0 });
