    ...
    grant: {
      execute: async (context: CLIContext): Promise<void> => {
        ...
        try {
          ...
          let user = await db.query<{ id: string; email: string }>(
            'SELECT id, email FROM auth_users WHERE email = ? OR id = ?',
            [args.user, args.user]
          ).then(rows => rows[0]);
          ...
          // Find role
          const role = await db.query<{ id: string; name: string }>(
            'SELECT id, name FROM auth_roles WHERE name = ?',
            [args.role]
          ).then(rows => rows[0]);
          ...
          const existing = await db.query(
            'SELECT 1 FROM auth_user_roles WHERE user_id = ? AND role_id = ?',
            [user.id, role.id]
          ...
          console.log(`Current roles: ${currentRoles.map(r => r.name).join(', ')}`);
      ...
      execute: async (context: CLIContext): Promise<void> => {
        ...
        try {
          ...
          if (currentRoles.length > 0) {
            console.log(`Remaining roles: ${currentRoles.map(r => r.name).join(', ')}`);
          } else {
