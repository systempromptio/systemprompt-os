...
export const command: ICLICommand = {
  ...
  execute: async (context: ICLIContext): Promise<void> => {
    ...
    try {
      ...
      logger.error(LogSource.AUTH, 'Command failed', { error });
      process.exit(1);
    }
  },
};
...
- `auth.module.generated.ts` - Create/Update data schemas
- `auth.service.generated.ts` - Service method schemas
...
```typescript
import { UserUpdateDataSchema } from '../types/auth.module.generated';
...
```typescript
// In auth/utils/cli-validation.ts
import { z } from 'zod';
import { CliOutputService } from '@/modules/core/cli/services/cli-output.service';
import { 
  UserCreateDataSchema,
...
3. **Type Generation**:
   - Run `./bin/systemprompt dev generate-types auth` after schema changes
