/**
 * @fileoverview OAuth Tunnel CLI Commands
 * @module modules/core/auth/cli/tunnel
 */
...
export async function startTunnel(): Promise<void> {
  ...
  try {
    const authModule = getAuthModule();
    await authModule.initialize({ logger: console });
...
if (process.argv[2] && commands[process.argv[2]]) {
  commands[process.argv[2]]().catch(console.error);
}