...
- `repositories/` - Data access layer
  - `auth.repository.ts` - Repository implementation
...
### index.ts Structure
- Must extend `BaseModule<IAuthModuleExports>`
...
import { AuthModuleExportsSchema } from './types/auth.module.generated';

// Module MUST validate its structure
const moduleSchema = createModuleSchema(AuthModuleExportsSchema);
...
- Location: `/tests/integration/modules/core/auth/auth.integration.test.ts`
...
- `auth.module.generated.ts` - Generated domain interfaces and Zod schemas from service methods
- `auth.service.generated.ts` - Generated service validation schemas from TypeScript AST
...
1. **Database Types & Schemas**
   ...
   export const AuthStatusSchema = z.nativeEnum(AuthStatus);
   ```
...
2. **Domain Types & Schemas**
   ...
   export const AuthUpdateDataSchema = z.object({ ... }).partial();
   
   // Type inference
   export type IAuthCreateData = z.infer<typeof AuthCreateDataSchema>;
   ```
...
1. **Service Methods**
   ...
   // Validate inputs
   async create(data: unknown): Promise<IAuth> {
     const validated = AuthCreateDataSchema.parse(data);
     const result = await this.repository.create(validated);
     return AuthRowSchema.parse(result); // Validate output
   }
   ```
...
2. **Repository Methods**
   ...
   async findById(id: string): Promise<IAuth | null> {
     ...
     return result ? AuthRowSchema.parse(result) : null;
   }
   ```
...
3. **CLI Commands**
   ...
   - See `/var/www/html/systemprompt-os/rules/src/modules/core/auth/cli/rules.md`
...
4. **Module Exports**
   ...
   get exports(): IAuthModuleExports {
     ...
     return AuthModuleExportsSchema.parse(exports);
   }
   ```
...
export class AuthService {
  ...
  async create(data: unknown): Promise<IAuth> {
    ...
    // Validate output
    return AuthRowSchema.parse(result);
  }
  
  async update(id: string, data: unknown): Promise<IAuth> {
    // Validate partial update data
    const validated = AuthUpdateDataSchema.parse(data);
    
    const result = await this.repository.update(id, validated);
    return AuthRowSchema.parse(result);
  }
  
  async list(filters: unknown): Promise<IAuth[]> {
    // Define and validate filter schema
    const filterSchema = z.object({
      status: AuthStatusSchema.optional(),
    ...
    return z.array(AuthRowSchema).parse(results);
  }
}
```
...
import { AuthRowSchema } from '../types/database.generated';

export class AuthRepository {
  async create(data: IAuthCreateData): Promise<IAuth> {
    const result = await this.db.query(
      `INSERT INTO auth (...) VALUES (...) RETURNING *`,
      [...]
    );
    
    // Validate database response
    return AuthRowSchema.parse(result.rows[0]);
  }
