...
/**
 * Authentication result interface using autogenerated types.
 */
...
export class AuthService {
  ...
  async login(input: ILoginInput): Promise<IAuthResult> {
    ...
    try {
      ...
      const session = await this.getSessionService().createSession({
        userId: user.id,
      ...
      return {
        ...
        refreshToken: session.refresh_token_hash || undefined,
        session,
      };
    } catch (error) {
   ...
   * @param input
   */
  async createApiToken(input: ITokenCreateInput): Promise<IAuthTokensRow> {
