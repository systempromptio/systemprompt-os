...
- `repositories/` - Data access layer
  - `cli.repository.ts` - Repository implementation
- `services/` - Business logic layer
  - `cli.service.ts` - Service implementation
...
### index.ts Structure
- Must extend `BaseModule<ICliModuleExports>`
...
import { CliModuleExportsSchema } from './types/cli.module.generated';

// Module MUST validate its structure
const moduleSchema = createModuleSchema(CliModuleExportsSchema);
...
```typescript
export class CliModule extends BaseModule<ICliModuleExports> {
...
- Location: `/tests/integration/modules/core/cli/cli.integration.test.ts`
...
- `cli.module.generated.ts` - Generated domain interfaces and Zod schemas from service methods
- `cli.service.generated.ts` - Generated service validation schemas from TypeScript AST

### What Gets Generated:
1. **Database Types & Schemas**
   ```typescript
   // Automatically generated from schema.sql
   export interface ICliRow { ... }
   export const CliRowSchema = z.object({ ... });
   
   // Enums with validation
   export enum CliStatus { ... }
   export const CliStatusSchema = z.nativeEnum(CliStatus);
   ```
...
2. **Domain Types & Schemas**
   ...
   export const CliUpdateDataSchema = z.object({ ... }).partial();
   
   // Type inference
   export type ICliCreateData = z.infer<typeof CliCreateDataSchema>;
   ```
...
./bin/systemprompt dev generate-types cli
```
...
1. **Service Methods**
   ...
   // Validate inputs
   async create(data: unknown): Promise<ICli> {
     const validated = CliCreateDataSchema.parse(data);
     const result = await this.repository.create(validated);
     return CliRowSchema.parse(result); // Validate output
   }
   ```
...
2. **Repository Methods**
   ...
   async findById(id: string): Promise<ICli | null> {
     const result = await this.db.query(...);
     return result ? CliRowSchema.parse(result) : null;
   }
   ```
...
3. **CLI Commands**
   ...
   - See `/var/www/html/systemprompt-os/rules/src/modules/core/cli/cli/rules.md`
...
4. **Module Exports**
   ...
   get exports(): ICliModuleExports {
     ...
     return CliModuleExportsSchema.parse(exports);
   }
   ```
...
export class CliService {
  ...
  async create(data: unknown): Promise<ICli> {
    ...
    // Validate output
    return CliRowSchema.parse(result);
  }
  
  async update(id: string, data: unknown): Promise<ICli> {
    // Validate partial update data
    const validated = CliUpdateDataSchema.parse(data);
    
    const result = await this.repository.update(id, validated);
    return CliRowSchema.parse(result);
  }
  
  async list(filters: unknown): Promise<ICli[]> {
    ...
    return z.array(CliRowSchema).parse(results);
  }
}
```
...
```typescript
// repositories/cli.repository.ts
import { CliRowSchema } from '../types/database.generated';

export class CliRepository {
  async create(data: ICliCreateData): Promise<ICli> {
    const result = await this.db.query(
      `INSERT INTO cli (...) VALUES (...) RETURNING *`,
      [...]
    );
    
    // Validate database response
    return CliRowSchema.parse(result.rows[0]);
  }
