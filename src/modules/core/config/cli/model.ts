    ...
    if (provider.name === 'google-liveapi') {
      ...
      Object.entries(config.models).forEach(([key, model]) => {
        ...
        if (model.tools && model.tools.length > 0) {
          const toolNames = model.tools.map(t => 
            t.codeExecution ? 'Code Execution' : 'Unknown'
          ).join(', ');
 ...
 * Test model
 */
async function testModel(options: { provider: string; model: string; prompt?: string }): Promise<void> {
  const provider = getProvider(options.provider);
