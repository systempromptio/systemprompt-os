...
3. **Database Storage**: During bootstrap, commands are registered in the `cli_commands` table with:
   - `command_path`: Full path like `config:command` (e.g., `config:create`)
...
```typescript
import { UserUpdateDataSchema } from '../types/config.module.generated';
...
```typescript
// In config/utils/cli-validation.ts
import { z } from 'zod';
import { CliOutputService } from '@/modules/core/cli/services/cli-output.service';
import { 
  UserCreateDataSchema,
...
3. **Type Generation**:
   - Run `./bin/systemprompt dev generate-types config` after schema changes
