...
async function execute(options: { file?: string }): Promise<void> {
  ...
  if (result.valid) {
    ...
    console.log('\nSummary:');
    console.log(`  Providers: ${config.providers?.available?.length || 0} available, ${config.providers?.enabled?.length || 0} enabled`);
    ...
    console.log(`  Server: ${config.defaults?.system?.host || 'localhost'}:${config.defaults?.system?.port || 8080}`);
  } else {
