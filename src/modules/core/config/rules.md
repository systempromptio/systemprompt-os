...
- `repositories/` - Data access layer
  - `config.repository.ts` - Repository implementation
- `services/` - Business logic layer
  - `config.service.ts` - Service implementation
...
### index.ts Structure
- Must extend `BaseModule<IConfigModuleExports>`
...
import { ConfigModuleExportsSchema } from './types/config.module.generated';

// Module MUST validate its structure
const moduleSchema = createModuleSchema(ConfigModuleExportsSchema);
...
- Location: `/tests/integration/modules/core/config/config.integration.test.ts`
...
- `config.module.generated.ts` - Generated domain interfaces and Zod schemas from service methods
- `config.service.generated.ts` - Generated service validation schemas from TypeScript AST
...
1. **Database Types & Schemas**
   ...
   export const ConfigStatusSchema = z.nativeEnum(ConfigStatus);
   ```
...
2. **Domain Types & Schemas**
   ...
   export const ConfigUpdateDataSchema = z.object({ ... }).partial();
   
   // Type inference
   export type IConfigCreateData = z.infer<typeof ConfigCreateDataSchema>;
   ```
...
./bin/systemprompt dev generate-types config
```
...
1. **Service Methods**
   ...
   // Validate inputs
   async create(data: unknown): Promise<IConfig> {
     const validated = ConfigCreateDataSchema.parse(data);
     const result = await this.repository.create(validated);
     return ConfigRowSchema.parse(result); // Validate output
   }
   ```
...
2. **Repository Methods**
   ...
   async findById(id: string): Promise<IConfig | null> {
     const result = await this.db.query(...);
     return result ? ConfigRowSchema.parse(result) : null;
   }
   ```
...
3. **CLI Commands**
   ...
   - See `/var/www/html/systemprompt-os/rules/src/modules/core/config/cli/rules.md`
...
4. **Module Exports**
   ...
   get exports(): IConfigModuleExports {
     ...
     return ConfigModuleExportsSchema.parse(exports);
   }
   ```
...
export class ConfigService {
  ...
  async create(data: unknown): Promise<IConfig> {
    ...
    // Validate output
    return ConfigRowSchema.parse(result);
  }
  
  async update(id: string, data: unknown): Promise<IConfig> {
    // Validate partial update data
    const validated = ConfigUpdateDataSchema.parse(data);
    
    const result = await this.repository.update(id, validated);
    return ConfigRowSchema.parse(result);
  }
  
  async list(filters: unknown): Promise<IConfig[]> {
    // Define and validate filter schema
    const filterSchema = z.object({
      status: ConfigStatusSchema.optional(),
    ...
    return z.array(ConfigRowSchema).parse(results);
  }
}
```
...
import { ConfigRowSchema } from '../types/database.generated';

export class ConfigRepository {
  async create(data: IConfigCreateData): Promise<IConfig> {
    const result = await this.db.query(
    ...
    return ConfigRowSchema.parse(result.rows[0]);
  }
