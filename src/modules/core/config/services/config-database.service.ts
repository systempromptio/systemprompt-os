...
export class ConfigDatabaseService {
  ...
  async getProvider(name: string): Promise<ConfigProvider | null> {
    ...
    if (results[0]) {
      results[0].config = typeof results[0].config === 'string' 
        ? JSON.parse(results[0].config) 
        : results[0].config;
      results[0].metadata = typeof results[0].metadata === 'string'
  ...
  async deleteProvider(name: string): Promise<boolean> {
    try {
      ...
      // Delete the provider
      await this.db.execute(
        'DELETE FROM config_providers WHERE name = ?',
        [name]
      );
      
      return true;
    } catch (error) {
      logger.error('Failed to delete provider', { name, error });
      return false;
    }
  }
  ...
  async queryProviders(query: ProviderQuery = {}): Promise<ConfigProvider[]> {
    ...
    return results.map(provider => ({
      ...
      metadata: typeof provider.metadata === 'string' ? JSON.parse(provider.metadata) : provider.metadata
    }));
  }
