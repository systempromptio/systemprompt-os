...
export class ConfigService {
  ...
  async createConfig(data: IConfigCreateData): Promise<IConfig> {
    ...
    return await this.repository.createConfig(encryptedData);
  }
}
```
...
import { ConfigStatus } from '@/modules/core/config/types/database.generated';
...
export class ConfigService {
  ...
  async createConfig(data: IConfigCreateData, userId: string): Promise<IConfig> {
    ...
    this.eventBus.on<UserDataRequestEvent>(
      ...
      async (event) => {
        try {
          ...
          this.logger?.error(LogSource.CONFIG, 'Failed to handle user data request', { error });
        }
      }
    );
  }
...
export class ConfigService {
  ...
  private constructor() {
    ...
    try {
      ...
      this.logger?.warn(LogSource.CONFIG, 'Audit service not available');
    }
  }
...
async createConfig(data: IConfigCreateData): Promise<IConfig> {
  ...
  await this.validateConfigData(data);

  const id = randomUUID();
  const now = new Date();

  this.logger?.info(LogSource.CONFIG, `Creating config: ${data.name}`);
  ...
  this.eventBus.emit(ConfigEvents.{ENTITY}_CREATED, {
    configId: config.id,
    timestamp: now
  });
...
### Validation Methods
```typescript
private async validateConfigData(data: IConfigCreateData): Promise<void> {
  ...
  if (existing) {
    throw new Error(`Config name already exists: ${data.name}`);
  }
...
```typescript
private setupEventHandlers(): void {
  this.eventBus.on<ConfigDataRequestEvent>(
    ConfigEvents.{ENTITY}_DATA_REQUEST, 
    async (data: unknown) => {
      try {
        const event = data as ConfigDataRequestEvent;
        const config = await this.getConfig(event.configId);
        
        const response: ConfigDataResponseEvent = {
          requestId: event.requestId,
          config: config ? { /* mapped data */ } : null
        };
        
        this.eventBus.emit(ConfigEvents.{ENTITY}_DATA_RESPONSE, response);
      } catch (error) {
        this.logger?.error(LogSource.CONFIG, 'Event handler error', { error });
...
if (data.status === 'inactive' && hasActiveRelations) {
  throw new Error('Cannot deactivate config with active relations');
}
...
### Log Sources
- **MUST** use module-specific LogSource (e.g., `LogSource.CONFIG`)
