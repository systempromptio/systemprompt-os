...
export function createRestoreCommand(db: DatabaseService, logger: ILogger): Command {
...
async function restoreDatabase(
  ...
  const statements = content
    .split(';')
    .map((s) => { return s.trim() })
    .filter((s) => { return s.length > 0 && !s.startsWith('--') });
