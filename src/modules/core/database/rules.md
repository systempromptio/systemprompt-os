...
- `services/` - Business logic layer
  - `database.service.ts` - Service implementation
...
./bin/systemprompt dev generate-types database
```
...
1. **Service Methods**
   ...
   // Validate inputs
   async create(data: unknown): Promise<IDatabase> {
     const validated = DatabaseCreateDataSchema.parse(data);
     const result = await this.repository.create(validated);
     return DatabaseRowSchema.parse(result); // Validate output
   }
   ```
...
2. **Repository Methods**
   ...
   async findById(id: string): Promise<IDatabase | null> {
     const result = await this.db.query(...);
     return result ? DatabaseRowSchema.parse(result) : null;
   }
   ```
...
export class DatabaseService {
  ...
  async create(data: unknown): Promise<IDatabase> {
    ...
    // Validate output
    return DatabaseRowSchema.parse(result);
  }
  
  async update(id: string, data: unknown): Promise<IDatabase> {
    // Validate partial update data
    const validated = DatabaseUpdateDataSchema.parse(data);
    
    const result = await this.repository.update(id, validated);
    return DatabaseRowSchema.parse(result);
  }
  
  async list(filters: unknown): Promise<IDatabase[]> {
    // Define and validate filter schema
    const filterSchema = z.object({
      status: DatabaseStatusSchema.optional(),
    ...
    return z.array(DatabaseRowSchema).parse(results);
  }
}
```
...
import { DatabaseRowSchema } from '../types/database.generated';

export class DatabaseRepository {
  async create(data: IDatabaseCreateData): Promise<IDatabase> {
    const result = await this.db.query(
      `INSERT INTO database (...) VALUES (...) RETURNING *`,
      [...]
    );
    
    // Validate database response
    return DatabaseRowSchema.parse(result.rows[0]);
  }
