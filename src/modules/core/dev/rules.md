...
- `repositories/` - Data access layer
  - `dev.repository.ts` - Repository implementation
- `services/` - Business logic layer
  - `dev.service.ts` - Service implementation
...
### index.ts Structure
- Must extend `BaseModule<IDevModuleExports>`
...
import { DevModuleExportsSchema } from './types/dev.module.generated';

// Module MUST validate its structure
const moduleSchema = createModuleSchema(DevModuleExportsSchema);
...
- Location: `/tests/integration/modules/core/dev/dev.integration.test.ts`
...
- `dev.module.generated.ts` - Generated domain interfaces and Zod schemas from service methods
- `dev.service.generated.ts` - Generated service validation schemas from TypeScript AST

### What Gets Generated:
1. **Database Types & Schemas**
   ```typescript
   // Automatically generated from schema.sql
   export interface IDevRow { ... }
   export const DevRowSchema = z.object({ ... });
   
   // Enums with validation
   export enum DevStatus { ... }
   export const DevStatusSchema = z.nativeEnum(DevStatus);
   ```
...
2. **Domain Types & Schemas**
   ...
   export const DevUpdateDataSchema = z.object({ ... }).partial();
   
   // Type inference
   export type IDevCreateData = z.infer<typeof DevCreateDataSchema>;
   ```
...
./bin/systemprompt dev generate-types dev
```
...
1. **Service Methods**
   ...
   // Validate inputs
   async create(data: unknown): Promise<IDev> {
     const validated = DevCreateDataSchema.parse(data);
     const result = await this.repository.create(validated);
     return DevRowSchema.parse(result); // Validate output
   }
   ```
...
2. **Repository Methods**
   ...
   async findById(id: string): Promise<IDev | null> {
     const result = await this.db.query(...);
     return result ? DevRowSchema.parse(result) : null;
   }
   ```
...
3. **CLI Commands**
   ...
   - See `/var/www/html/systemprompt-os/rules/src/modules/core/dev/cli/rules.md`
...
4. **Module Exports**
   ...
   get exports(): IDevModuleExports {
     ...
     return DevModuleExportsSchema.parse(exports);
   }
   ```
...
export class DevService {
  ...
  async create(data: unknown): Promise<IDev> {
    ...
    // Validate output
    return DevRowSchema.parse(result);
  }
  
  async update(id: string, data: unknown): Promise<IDev> {
    // Validate partial update data
    const validated = DevUpdateDataSchema.parse(data);
    
    const result = await this.repository.update(id, validated);
    return DevRowSchema.parse(result);
  }
  
  async list(filters: unknown): Promise<IDev[]> {
    ...
    return z.array(DevRowSchema).parse(results);
  }
}
```
...
```typescript
// repositories/dev.repository.ts
import { DevRowSchema } from '../types/database.generated';

export class DevRepository {
  async create(data: IDevCreateData): Promise<IDev> {
    const result = await this.db.query(
      `INSERT INTO dev (...) VALUES (...) RETURNING *`,
      [...]
    );
    
    // Validate database response
    return DevRowSchema.parse(result.rows[0]);
  }
