...
### dev.service.ts (MANDATORY)
**MUST** be named exactly as the module name (e.g., `dev.service.ts` for dev module). This is the primary service that implements the main module interface.
**MUST** be located directly in the services directory (e.g., `/services/dev.service.ts`), NOT inside a subfolder.
...
### Main Module Service
The primary service (e.g., `dev.service.ts`) serves as:
...
```
services/
├── dev.service.ts           # Main module service (MANDATORY - at root level)
├── validation.service.ts         # Additional service (at root level)
├── dev/                     # Optional support folder for dev.service.ts
│   ├── helpers.ts
│   ├── validators.ts
│   └── types.ts
...
export class DevService {
  ...
  async createDev(data: IDevCreateData): Promise<IDev> {
    ...
    return await this.repository.createDev(encryptedData);
  }
}
```
...
import {
  ...
  type IDevCreateData,
  type IDevUpdateData
} from '@/modules/core/dev/types/dev.module.generated';
import type { IDevService } from '@/modules/core/dev/types/dev.service.generated';
import { DevStatus } from '@/modules/core/dev/types/database.generated';
...
- **MUST** implement the auto-generated service interface from `types/dev.service.generated.ts` (main service only)
...
- **MUST** use `DevRepository.getInstance()` for data access (main service only)
...
export class DevService {
  ...
  async createDev(data: IDevCreateData, userId: string): Promise<IDev> {
    ...
    this.eventBus.on<UserDataRequestEvent>(
      ...
      async (event) => {
        try {
          ...
          this.logger?.error(LogSource.DEV, 'Failed to handle user data request', { error });
        }
      }
    );
  }
...
export class DevService {
  ...
  private constructor() {
    ...
    try {
      ...
      this.logger?.warn(LogSource.DEV, 'Audit service not available');
    }
  }
...
async createDev(data: IDevCreateData): Promise<IDev> {
  ...
  await this.validateDevData(data);

  const id = randomUUID();
  const now = new Date();

  this.logger?.info(LogSource.DEV, `Creating dev: ${data.name}`);

  const dev = await this.repository.createDev({
    id,
    ...data,
  ...
  this.eventBus.emit(DevEvents.{ENTITY}_CREATED, {
    devId: dev.id,
    timestamp: now
  });
...
### Validation Methods
```typescript
private async validateDevData(data: IDevCreateData): Promise<void> {
  // Business rule validation
  if (!data.name?.trim()) {
    throw new Error('Dev name is required');
  }

  // Uniqueness validation
  const existing = await this.repository.findByName(data.name);
  if (existing) {
    throw new Error(`Dev name already exists: ${data.name}`);
  }
...
```typescript
private setupEventHandlers(): void {
  this.eventBus.on<DevDataRequestEvent>(
    DevEvents.{ENTITY}_DATA_REQUEST, 
    async (data: unknown) => {
      try {
        const event = data as DevDataRequestEvent;
        const dev = await this.getDev(event.devId);
        
        const response: DevDataResponseEvent = {
          requestId: event.requestId,
          dev: dev ? { /* mapped data */ } : null
        };
        
        this.eventBus.emit(DevEvents.{ENTITY}_DATA_RESPONSE, response);
      } catch (error) {
        this.logger?.error(LogSource.DEV, 'Event handler error', { error });
...
### Generated Types
- **MUST** use auto-generated interfaces from `types/dev.module.generated.ts`
...
### Return Types
- **MUST** return domain interfaces (e.g., `IDev`), not database rows
...
if (data.status === 'inactive' && hasActiveRelations) {
  throw new Error('Cannot deactivate dev with active relations');
}

// Not found errors
const dev = await this.repository.findById(id);
if (!dev) {
  throw new Error(`Dev not found: ${id}`);
}
```
...
```typescript
// Operation start
this.logger?.info(LogSource.DEV, `Starting operation: ${operation}`);

// Success
this.logger?.info(LogSource.DEV, `Operation completed: ${operation}`);
...
### Log Sources
- **MUST** use module-specific LogSource (e.g., `LogSource.DEV`)
