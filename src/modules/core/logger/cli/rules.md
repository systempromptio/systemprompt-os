...
./bin/systemprompt logger {command}
...
- **MUST** follow the exact pattern:
  ...
  export const loggerCommands = {
    name: 'logger',
    alias: '{shortname}',  
    description: 'Logger module commands',
    subcommands: [/* command metadata */]
  };
  export default loggerCommands;
  ```
...
export const command: ICLICommand = {
  ...
  execute: async (context: ICLIContext): Promise<void> => {
    ...
    try {
      ...
      logger.error(LogSource.LOGGER, 'Command failed', { error });
      process.exit(1);
    }
  },
};
...
2. **Command Implementation**: Each command is implemented in `src/modules/core/logger/cli/{command}.ts`
3. **Database Storage**: During bootstrap, commands are registered in the `cli_commands` table with:
   - `command_path`: Full path like `logger:command` (e.g., `logger:create`)
   - `executor_path`: Path to the TypeScript file implementing the command
   - Command metadata (description, options, etc.)
4. **CLI Execution**: When running `systemprompt logger {command}`:
   - The CLI main.ts loads commands from the database
   - Parses the command path (`logger:{command}`) to find the executor
   - Dynamically imports the command module
   - Executes the command with parsed context

### Command Handler Resolution
- The handler string in `cli/index.ts` (e.g., `handler: 'logger:{command}'`) maps to the command path
...
- `logger.module.generated.ts` - Create/Update data schemas
- `logger.service.generated.ts` - Service method schemas
...
```typescript
import { UserUpdateDataSchema } from '../types/logger.module.generated';
...
```typescript
// In logger/utils/cli-validation.ts
import { z } from 'zod';
import { CliOutputService } from '@/modules/core/cli/services/cli-output.service';
import { 
  UserCreateDataSchema,
...
3. **Type Generation**:
   - Run `./bin/systemprompt dev generate-types logger` after schema changes
