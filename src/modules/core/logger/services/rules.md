...
### logger.service.ts (MANDATORY)
**MUST** be named exactly as the module name (e.g., `logger.service.ts` for logger module). This is the primary service that implements the main module interface.
**MUST** be located directly in the services directory (e.g., `/services/logger.service.ts`), NOT inside a subfolder.
...
### Main Module Service
The primary service (e.g., `logger.service.ts`) serves as:
...
```
services/
├── logger.service.ts           # Main module service (MANDATORY - at root level)
├── validation.service.ts         # Additional service (at root level)
├── logger/                     # Optional support folder for logger.service.ts
│   ├── helpers.ts
│   ├── validators.ts
│   └── types.ts
...
export class LoggerService {
  ...
  private constructor() {
    this.repository = LoggerRepository.getInstance();
    this.validationService = ValidationService.getInstance();
    this.encryptionService = EncryptionService.getInstance();
  }
  
  async createLogger(data: ILoggerCreateData): Promise<ILogger> {
    ...
    return await this.repository.createLogger(encryptedData);
  }
}
```
...
import { LoggerRepository } from '@/modules/core/logger/repositories/logger.repository';
import { EventBusService } from '@/modules/core/events/services/event-bus.service';
import {
  type ILogger,
  type ILoggerCreateData,
  type ILoggerUpdateData
} from '@/modules/core/logger/types/logger.module.generated';
import type { ILoggerService } from '@/modules/core/logger/types/logger.service.generated';
import { LoggerStatus } from '@/modules/core/logger/types/database.generated';
...
export class LoggerService implements ILoggerService {
  ...
  async initialize(): Promise<void> {
    ...
    this.logger?.info(LogSource.LOGGER, 'LoggerService initialized');
  }
...
- **MUST** implement the auto-generated service interface from `types/logger.service.generated.ts` (main service only)
...
- **MUST** use `LoggerRepository.getInstance()` for data access (main service only)
...
export class LoggerService {
  ...
  async createLogger(data: ILoggerCreateData, userId: string): Promise<ILogger> {
    ...
    this.eventBus.on<UserDataRequestEvent>(
      ...
      async (event) => {
        try {
          ...
          this.logger?.error(LogSource.LOGGER, 'Failed to handle user data request', { error });
        }
      }
    );
  }
...
export class LoggerService {
  ...
  private constructor() {
    ...
    try {
      ...
      this.logger?.warn(LogSource.LOGGER, 'Audit service not available');
    }
  }
...
async createLogger(data: ILoggerCreateData): Promise<ILogger> {
  ...
  await this.validateLoggerData(data);

  const id = randomUUID();
  const now = new Date();

  this.logger?.info(LogSource.LOGGER, `Creating logger: ${data.name}`);

  const logger = await this.repository.createLogger({
    id,
    ...data,
  ...
  this.eventBus.emit(LoggerEvents.{ENTITY}_CREATED, {
    loggerId: logger.id,
    timestamp: now
  });
...
### Validation Methods
```typescript
private async validateLoggerData(data: ILoggerCreateData): Promise<void> {
  ...
  if (existing) {
    throw new Error(`Logger name already exists: ${data.name}`);
  }
...
```typescript
private setupEventHandlers(): void {
  this.eventBus.on<LoggerDataRequestEvent>(
    LoggerEvents.{ENTITY}_DATA_REQUEST, 
    async (data: unknown) => {
      try {
        const event = data as LoggerDataRequestEvent;
        const logger = await this.getLogger(event.loggerId);
        
        const response: LoggerDataResponseEvent = {
          requestId: event.requestId,
          logger: logger ? { /* mapped data */ } : null
        };
        
        this.eventBus.emit(LoggerEvents.{ENTITY}_DATA_RESPONSE, response);
      } catch (error) {
        this.logger?.error(LogSource.LOGGER, 'Event handler error', { error });
...
### Generated Types
- **MUST** use auto-generated interfaces from `types/logger.module.generated.ts`
...
### Return Types
- **MUST** return domain interfaces (e.g., `ILogger`), not database rows
...
if (data.status === 'inactive' && hasActiveRelations) {
  throw new Error('Cannot deactivate logger with active relations');
}

// Not found errors
const logger = await this.repository.findById(id);
if (!logger) {
  throw new Error(`Logger not found: ${id}`);
}
```
...
```typescript
// Operation start
this.logger?.info(LogSource.LOGGER, `Starting operation: ${operation}`);

// Success
this.logger?.info(LogSource.LOGGER, `Operation completed: ${operation}`);

// Errors
this.logger?.error(LogSource.LOGGER, 'Operation failed', {
...
### Log Sources
- **MUST** use module-specific LogSource (e.g., `LogSource.LOGGER`)
