...
- `repositories/` - Data access layer
  - `mcp.repository.ts` - Repository implementation
- `services/` - Business logic layer
  - `mcp.service.ts` - Service implementation
...
### index.ts Structure
- Must extend `BaseModule<IMcpModuleExports>`
...
import { McpModuleExportsSchema } from './types/mcp.module.generated';

// Module MUST validate its structure
const moduleSchema = createModuleSchema(McpModuleExportsSchema);
...
- Location: `/tests/integration/modules/core/mcp/mcp.integration.test.ts`
...
- `mcp.module.generated.ts` - Generated domain interfaces and Zod schemas from service methods
- `mcp.service.generated.ts` - Generated service validation schemas from TypeScript AST

### What Gets Generated:
1. **Database Types & Schemas**
   ```typescript
   // Automatically generated from schema.sql
   export interface IMcpRow { ... }
   export const McpRowSchema = z.object({ ... });
   
   // Enums with validation
   export enum McpStatus { ... }
   export const McpStatusSchema = z.nativeEnum(McpStatus);
   ```
...
2. **Domain Types & Schemas**
   ...
   export const McpUpdateDataSchema = z.object({ ... }).partial();
   
   // Type inference
   export type IMcpCreateData = z.infer<typeof McpCreateDataSchema>;
   ```
...
./bin/systemprompt dev generate-types mcp
```
...
1. **Service Methods**
   ...
   // Validate inputs
   async create(data: unknown): Promise<IMcp> {
     const validated = McpCreateDataSchema.parse(data);
     const result = await this.repository.create(validated);
     return McpRowSchema.parse(result); // Validate output
   }
   ```
...
2. **Repository Methods**
   ...
   async findById(id: string): Promise<IMcp | null> {
     const result = await this.db.query(...);
     return result ? McpRowSchema.parse(result) : null;
   }
   ```
...
3. **CLI Commands**
   ...
   - See `/var/www/html/systemprompt-os/rules/src/modules/core/mcp/cli/rules.md`
...
4. **Module Exports**
   ...
   get exports(): IMcpModuleExports {
     ...
     return McpModuleExportsSchema.parse(exports);
   }
   ```
...
export class McpService {
  ...
  async create(data: unknown): Promise<IMcp> {
    ...
    // Validate output
    return McpRowSchema.parse(result);
  }
  
  async update(id: string, data: unknown): Promise<IMcp> {
    // Validate partial update data
    const validated = McpUpdateDataSchema.parse(data);
    
    const result = await this.repository.update(id, validated);
    return McpRowSchema.parse(result);
  }
  
  async list(filters: unknown): Promise<IMcp[]> {
    ...
    return z.array(McpRowSchema).parse(results);
  }
}
```
...
```typescript
// repositories/mcp.repository.ts
import { McpRowSchema } from '../types/database.generated';

export class McpRepository {
  async create(data: IMcpCreateData): Promise<IMcp> {
    const result = await this.db.query(
      `INSERT INTO mcp (...) VALUES (...) RETURNING *`,
      [...]
    );
    
    // Validate database response
    return McpRowSchema.parse(result.rows[0]);
  }
  
  async findById(id: string): Promise<IMcp | null> {
    const result = await this.db.query(
      `SELECT * FROM mcp WHERE id = $1`,
      [id]
    );
