...
### mcp.service.ts (MANDATORY)
**MUST** be named exactly as the module name (e.g., `mcp.service.ts` for mcp module). This is the primary service that implements the main module interface.
**MUST** be located directly in the services directory (e.g., `/services/mcp.service.ts`), NOT inside a subfolder.
...
### Main Module Service
The primary service (e.g., `mcp.service.ts`) serves as:
...
```
services/
├── mcp.service.ts           # Main module service (MANDATORY - at root level)
├── validation.service.ts         # Additional service (at root level)
├── mcp/                     # Optional support folder for mcp.service.ts
│   ├── helpers.ts
│   ├── validators.ts
│   └── types.ts
...
export class McpService {
  ...
  async createMcp(data: IMcpCreateData): Promise<IMcp> {
    ...
    return await this.repository.createMcp(encryptedData);
  }
}
```
...
import type { IMcpService } from '@/modules/core/mcp/types/mcp.service.generated';
import { McpStatus } from '@/modules/core/mcp/types/database.generated';
...
- **MUST** implement the auto-generated service interface from `types/mcp.service.generated.ts` (main service only)
...
- **MUST** use `McpRepository.getInstance()` for data access (main service only)
...
export class McpService {
  ...
  async createMcp(data: IMcpCreateData, userId: string): Promise<IMcp> {
    ...
    this.eventBus.on<UserDataRequestEvent>(
      ...
      async (event) => {
        try {
          ...
          this.logger?.error(LogSource.MCP, 'Failed to handle user data request', { error });
        }
      }
    );
  }
...
export class McpService {
  ...
  private constructor() {
    ...
    try {
      ...
      this.logger?.warn(LogSource.MCP, 'Audit service not available');
    }
  }
...
async createMcp(data: IMcpCreateData): Promise<IMcp> {
  ...
  await this.validateMcpData(data);

  const id = randomUUID();
  const now = new Date();

  this.logger?.info(LogSource.MCP, `Creating mcp: ${data.name}`);

  const mcp = await this.repository.createMcp({
    id,
    ...data,
  ...
  this.eventBus.emit(McpEvents.{ENTITY}_CREATED, {
    mcpId: mcp.id,
    timestamp: now
  });
...
### Validation Methods
```typescript
private async validateMcpData(data: IMcpCreateData): Promise<void> {
  ...
  if (existing) {
    throw new Error(`Mcp name already exists: ${data.name}`);
  }
...
```typescript
private setupEventHandlers(): void {
  this.eventBus.on<McpDataRequestEvent>(
    McpEvents.{ENTITY}_DATA_REQUEST, 
    async (data: unknown) => {
      try {
        const event = data as McpDataRequestEvent;
        const mcp = await this.getMcp(event.mcpId);
        
        const response: McpDataResponseEvent = {
          requestId: event.requestId,
          mcp: mcp ? { /* mapped data */ } : null
        };
        
        this.eventBus.emit(McpEvents.{ENTITY}_DATA_RESPONSE, response);
      } catch (error) {
        this.logger?.error(LogSource.MCP, 'Event handler error', { error });
...
### Generated Types
- **MUST** use auto-generated interfaces from `types/mcp.module.generated.ts`
...
### Return Types
- **MUST** return domain interfaces (e.g., `IMcp`), not database rows
...
if (data.status === 'inactive' && hasActiveRelations) {
  throw new Error('Cannot deactivate mcp with active relations');
}
...
```typescript
// Operation start
this.logger?.info(LogSource.MCP, `Starting operation: ${operation}`);

// Success
this.logger?.info(LogSource.MCP, `Operation completed: ${operation}`);

// Errors
this.logger?.error(LogSource.MCP, 'Operation failed', {
...
### Log Sources
- **MUST** use module-specific LogSource (e.g., `LogSource.MCP`)
