...
./bin/systemprompt modules {command}
...
- **MUST** follow the exact pattern:
  ...
  export const modulesCommands = {
    name: 'modules',
    alias: '{shortname}',  
    description: 'Modules module commands',
    subcommands: [/* command metadata */]
  };
  export default modulesCommands;
  ```
...
export const command: ICLICommand = {
  ...
  execute: async (context: ICLIContext): Promise<void> => {
    ...
    try {
      const service = ModulesService.getInstance();
      ...
      logger.error(LogSource.MODULES, 'Command failed', { error });
      process.exit(1);
    }
  },
};
...
2. **Command Implementation**: Each command is implemented in `src/modules/core/modules/cli/{command}.ts`
3. **Database Storage**: During bootstrap, commands are registered in the `cli_commands` table with:
   - `command_path`: Full path like `modules:command` (e.g., `modules:create`)
   - `executor_path`: Path to the TypeScript file implementing the command
   - Command metadata (description, options, etc.)
4. **CLI Execution**: When running `systemprompt modules {command}`:
   - The CLI main.ts loads commands from the database
   - Parses the command path (`modules:{command}`) to find the executor
   - Dynamically imports the command module
   - Executes the command with parsed context

### Command Handler Resolution
- The handler string in `cli/index.ts` (e.g., `handler: 'modules:{command}'`) maps to the command path
...
- `modules.module.generated.ts` - Create/Update data schemas
- `modules.service.generated.ts` - Service method schemas
...
```typescript
import { UserUpdateDataSchema } from '../types/modules.module.generated';
...
```typescript
// In modules/utils/cli-validation.ts
import { z } from 'zod';
import { CliOutputService } from '@/modules/core/cli/services/cli-output.service';
import { 
  UserCreateDataSchema,
...
3. **Type Generation**:
   - Run `./bin/systemprompt dev generate-types modules` after schema changes
