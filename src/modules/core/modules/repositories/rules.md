...
### modules.repository.ts (MANDATORY)
**MUST** be named exactly as the service name (e.g., `modules.repository.ts` for `ModulesService`).
...
import { type IModulesRow, ModulesStatus } from '@/modules/core/modules/types/database.generated';
...
async createModule(moduleData: /* Auto-generated type subset */): Promise<IModulesRow> {
...
```typescript
async findById(id: string): Promise<IModulesRow | null> {
  const database = await this.getDatabase();
  const result = await database.query<IModulesRow>(
...
async findAll(): Promise<IModulesRow[]> {
  const database = await this.getDatabase();
  const result = await database.query<IModulesRow>(
...
```typescript
async updateModule(id: string, data: /* Auto-generated update type subset */): Promise<IModulesRow> {
  const database = await this.getDatabase();
  ...
  const stmt = await database.prepare(
    `UPDATE module SET ${updates.join(', ')} WHERE id = ?`
  );
  ...
  if (!module) {
    throw new Error(`Module not found after update: ${id}`);
  }
  return module;
}
```
...
async deleteModule(id: string): Promise<void> {
  const database = await this.getDatabase();
  const stmt = await database.prepare('DELETE FROM module WHERE id = ?');
  await stmt.run([id]);
  await stmt.finalize();
}
```
...
- **MUST** import table row interfaces (e.g., `IModulesRow`)
- **MUST** import enum types for status fields (e.g., `ModulesStatus`)
...
- **MUST** import from `types/modules.module.generated.ts` (auto-generated from service analysis)
- **MUST** use exact parameter types matching service method definitions

### Return Types (Generated from Service Analysis)
- **MUST** return database row types directly (`Promise<IModulesRow | null>`)
- **MUST NOT** transform to domain types (services handle transformation)
- **MUST** return `Promise<IModulesRow[]>` for list operations
- **MUST** return `Promise<IModulesRow>` for create/update operations
...
1. **SQL Schema Analysis** (`database/schema.sql`)
   - Generates `IModulesRow` interface with exact column types
