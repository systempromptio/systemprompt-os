...
### modules.service.ts (MANDATORY)
**MUST** be named exactly as the module name (e.g., `modules.service.ts` for modules module). This is the primary service that implements the main module interface.
**MUST** be located directly in the services directory (e.g., `/services/modules.service.ts`), NOT inside a subfolder.
...
### Main Module Service
The primary service (e.g., `modules.service.ts`) serves as:
...
```
services/
├── modules.service.ts           # Main module service (MANDATORY - at root level)
├── validation.service.ts         # Additional service (at root level)
├── modules/                     # Optional support folder for modules.service.ts
│   ├── helpers.ts
│   ├── validators.ts
│   └── types.ts
...
export class ModulesService {
  ...
  private constructor() {
    this.repository = ModulesRepository.getInstance();
    this.validationService = ValidationService.getInstance();
    this.encryptionService = EncryptionService.getInstance();
  }
  
  async createModule(data: IModuleCreateData): Promise<IModule> {
    ...
    return await this.repository.createModule(encryptedData);
  }
}
```
...
import { ModulesRepository } from '@/modules/core/modules/repositories/modules.repository';
import { EventBusService } from '@/modules/core/events/services/event-bus.service';
import {
  type IModule,
  type IModuleCreateData,
  type IModuleUpdateData
} from '@/modules/core/modules/types/modules.module.generated';
import type { IModulesService } from '@/modules/core/modules/types/modules.service.generated';
...
export class ModulesService implements IModulesService {
  private static instance: ModulesService;
  private readonly repository: ModulesRepository;
  ...
  static getInstance(): ModulesService {
    ModulesService.instance ||= new ModulesService();
  ...
  async initialize(): Promise<void> {
    ...
    this.logger?.info(LogSource.MODULES, 'ModulesService initialized');
  }
...
- **MUST** implement the auto-generated service interface from `types/modules.service.generated.ts` (main service only)
...
- **MUST** use `ModulesRepository.getInstance()` for data access (main service only)
...
export class ModulesService {
  ...
  async createModule(data: IModuleCreateData, userId: string): Promise<IModule> {
    ...
    this.eventBus.on<UserDataRequestEvent>(
      ...
      async (event) => {
        try {
          ...
          this.logger?.error(LogSource.MODULES, 'Failed to handle user data request', { error });
        }
      }
    );
  }
...
export class ModulesService {
  ...
  private constructor() {
    ...
    try {
      ...
      this.logger?.warn(LogSource.MODULES, 'Audit service not available');
    }
  }
...
async createModule(data: IModuleCreateData): Promise<IModule> {
  ...
  await this.validateModuleData(data);

  const id = randomUUID();
  const now = new Date();

  this.logger?.info(LogSource.MODULES, `Creating module: ${data.name}`);

  const module = await this.repository.createModule({
    id,
    ...data,
    status: ModulesStatus.ACTIVE,
    created_at: now.toISOString(),
    updated_at: now.toISOString()
  });

  // Emit events for other modules
  this.eventBus.emit(ModulesEvents.{ENTITY}_CREATED, {
    moduleId: module.id,
    timestamp: now
  });
...
### Validation Methods
```typescript
private async validateModuleData(data: IModuleCreateData): Promise<void> {
  ...
  if (existing) {
    throw new Error(`Module name already exists: ${data.name}`);
  }
...
```typescript
private setupEventHandlers(): void {
  this.eventBus.on<ModuleDataRequestEvent>(
    ModulesEvents.{ENTITY}_DATA_REQUEST, 
    async (data: unknown) => {
      try {
        const event = data as ModuleDataRequestEvent;
        const module = await this.getModule(event.moduleId);
        
        const response: ModuleDataResponseEvent = {
          requestId: event.requestId,
          module: module ? { /* mapped data */ } : null
        };
        
        this.eventBus.emit(ModulesEvents.{ENTITY}_DATA_RESPONSE, response);
      } catch (error) {
        this.logger?.error(LogSource.MODULES, 'Event handler error', { error });
...
### Generated Types
- **MUST** use auto-generated interfaces from `types/modules.module.generated.ts`
...
### Return Types
- **MUST** return domain interfaces (e.g., `IModule`), not database rows
...
if (data.status === 'inactive' && hasActiveRelations) {
  throw new Error('Cannot deactivate module with active relations');
}
...
```typescript
// Operation start
this.logger?.info(LogSource.MODULES, `Starting operation: ${operation}`);

// Success
this.logger?.info(LogSource.MODULES, `Operation completed: ${operation}`);

// Errors
this.logger?.error(LogSource.MODULES, 'Operation failed', {
...
### Log Sources
- **MUST** use module-specific LogSource (e.g., `LogSource.MODULES`)
