...
./bin/systemprompt monitor {command}
...
- **MUST** follow the exact pattern:
  ...
  export const monitorCommands = {
    name: 'monitor',
    alias: '{shortname}',  
    description: 'Monitor module commands',
...
export const command: ICLICommand = {
  ...
  execute: async (context: ICLIContext): Promise<void> => {
    ...
    try {
      ...
      logger.error(LogSource.MONITOR, 'Command failed', { error });
      process.exit(1);
    }
  },
};
...
2. **Command Implementation**: Each command is implemented in `src/modules/core/monitor/cli/{command}.ts`
3. **Database Storage**: During bootstrap, commands are registered in the `cli_commands` table with:
   - `command_path`: Full path like `monitor:command` (e.g., `monitor:create`)
   - `executor_path`: Path to the TypeScript file implementing the command
   - Command metadata (description, options, etc.)
4. **CLI Execution**: When running `systemprompt monitor {command}`:
   - The CLI main.ts loads commands from the database
   - Parses the command path (`monitor:{command}`) to find the executor
   - Dynamically imports the command module
   - Executes the command with parsed context

### Command Handler Resolution
- The handler string in `cli/index.ts` (e.g., `handler: 'monitor:{command}'`) maps to the command path
...
- `monitor.module.generated.ts` - Create/Update data schemas
- `monitor.service.generated.ts` - Service method schemas
...
} from '../types/monitor.module.generated';
...
```typescript
import { UserUpdateDataSchema } from '../types/monitor.module.generated';
...
```typescript
// In monitor/utils/cli-validation.ts
import { z } from 'zod';
import { CliOutputService } from '@/modules/core/cli/services/cli-output.service';
import { 
  UserCreateDataSchema,
...
3. **Type Generation**:
   - Run `./bin/systemprompt dev generate-types monitor` after schema changes
