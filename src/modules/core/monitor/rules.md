...
- `repositories/` - Data access layer
  - `monitor.repository.ts` - Repository implementation
- `services/` - Business logic layer
  - `monitor.service.ts` - Service implementation
...
### index.ts Structure
- Must extend `BaseModule<IMonitorModuleExports>`
...
import { MonitorModuleExportsSchema } from './types/monitor.module.generated';

// Module MUST validate its structure
const moduleSchema = createModuleSchema(MonitorModuleExportsSchema);
...
- Location: `/tests/integration/modules/core/monitor/monitor.integration.test.ts`
...
- `monitor.module.generated.ts` - Generated domain interfaces and Zod schemas from service methods
- `monitor.service.generated.ts` - Generated service validation schemas from TypeScript AST

### What Gets Generated:
1. **Database Types & Schemas**
   ```typescript
   // Automatically generated from schema.sql
   export interface IMonitorRow { ... }
   export const MonitorRowSchema = z.object({ ... });
   
   // Enums with validation
   export enum MonitorStatus { ... }
   export const MonitorStatusSchema = z.nativeEnum(MonitorStatus);
   ```
...
2. **Domain Types & Schemas**
   ...
   export const MonitorUpdateDataSchema = z.object({ ... }).partial();
   
   // Type inference
   export type IMonitorCreateData = z.infer<typeof MonitorCreateDataSchema>;
   ```
...
./bin/systemprompt dev generate-types monitor
```
...
1. **Service Methods**
   ...
   import { 
     MonitorCreateDataSchema,
     MonitorUpdateDataSchema,
     MonitorRowSchema 
   } from '../types/monitor.module.generated';
   
   // Validate inputs
   async create(data: unknown): Promise<IMonitor> {
     const validated = MonitorCreateDataSchema.parse(data);
     const result = await this.repository.create(validated);
     return MonitorRowSchema.parse(result); // Validate output
   }
   ```
...
2. **Repository Methods**
   ...
   async findById(id: string): Promise<IMonitor | null> {
     ...
     return result ? MonitorRowSchema.parse(result) : null;
   }
   ```
...
3. **CLI Commands**
   ...
   - See `/var/www/html/systemprompt-os/rules/src/modules/core/monitor/cli/rules.md`
...
4. **Module Exports**
   ...
   get exports(): IMonitorModuleExports {
     ...
     return MonitorModuleExportsSchema.parse(exports);
   }
   ```
...
export class MonitorService {
  ...
  async create(data: unknown): Promise<IMonitor> {
    ...
    // Validate output
    return MonitorRowSchema.parse(result);
  }
  
  async update(id: string, data: unknown): Promise<IMonitor> {
    // Validate partial update data
    const validated = MonitorUpdateDataSchema.parse(data);
    
    const result = await this.repository.update(id, validated);
    return MonitorRowSchema.parse(result);
  }
  
  async list(filters: unknown): Promise<IMonitor[]> {
    ...
    return z.array(MonitorRowSchema).parse(results);
  }
}
```
...
import { MonitorRowSchema } from '../types/database.generated';

export class MonitorRepository {
  async create(data: IMonitorCreateData): Promise<IMonitor> {
    const result = await this.db.query(
      `INSERT INTO monitor (...) VALUES (...) RETURNING *`,
      [...]
    );
    
    // Validate database response
    return MonitorRowSchema.parse(result.rows[0]);
  }
