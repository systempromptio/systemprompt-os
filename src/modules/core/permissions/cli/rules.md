...
./bin/systemprompt permissions {command}
...
- **MUST** follow the exact pattern:
  ...
  export const permissionsCommands = {
    name: 'permissions',
    alias: '{shortname}',  
    description: 'Permissions module commands',
    subcommands: [/* command metadata */]
  };
  export default permissionsCommands;
  ```
...
export const command: ICLICommand = {
  ...
  execute: async (context: ICLIContext): Promise<void> => {
    ...
    try {
      ...
      logger.error(LogSource.PERMISSIONS, 'Command failed', { error });
      process.exit(1);
    }
  },
};
...
2. **Command Implementation**: Each command is implemented in `src/modules/core/permissions/cli/{command}.ts`
3. **Database Storage**: During bootstrap, commands are registered in the `cli_commands` table with:
   - `command_path`: Full path like `permissions:command` (e.g., `permissions:create`)
   - `executor_path`: Path to the TypeScript file implementing the command
   - Command metadata (description, options, etc.)
4. **CLI Execution**: When running `systemprompt permissions {command}`:
   - The CLI main.ts loads commands from the database
   - Parses the command path (`permissions:{command}`) to find the executor
   - Dynamically imports the command module
   - Executes the command with parsed context

### Command Handler Resolution
- The handler string in `cli/index.ts` (e.g., `handler: 'permissions:{command}'`) maps to the command path
...
- `permissions.module.generated.ts` - Create/Update data schemas
- `permissions.service.generated.ts` - Service method schemas
...
} from '../types/permissions.module.generated';
...
```typescript
import { UserUpdateDataSchema } from '../types/permissions.module.generated';
...
```typescript
// In permissions/utils/cli-validation.ts
import { z } from 'zod';
import { CliOutputService } from '@/modules/core/cli/services/cli-output.service';
import { 
  UserCreateDataSchema,
...
3. **Type Generation**:
   - Run `./bin/systemprompt dev generate-types permissions` after schema changes
