...
- `repositories/` - Data access layer
  - `permissions.repository.ts` - Repository implementation
- `services/` - Business logic layer
  - `permissions.service.ts` - Service implementation
...
### index.ts Structure
- Must extend `BaseModule<IPermissionsModuleExports>`
...
import { PermissionsModuleExportsSchema } from './types/permissions.module.generated';

// Module MUST validate its structure
const moduleSchema = createModuleSchema(PermissionsModuleExportsSchema);
...
- Location: `/tests/integration/modules/core/permissions/permissions.integration.test.ts`
...
- `permissions.module.generated.ts` - Generated domain interfaces and Zod schemas from service methods
- `permissions.service.generated.ts` - Generated service validation schemas from TypeScript AST
...
1. **Database Types & Schemas**
   ...
   export const PermissionsStatusSchema = z.nativeEnum(PermissionsStatus);
   ```

2. **Domain Types & Schemas**
   ```typescript
   // Create/Update data schemas
   export const PermissionsCreateDataSchema = z.object({ ... });
   export const PermissionsUpdateDataSchema = z.object({ ... }).partial();
   
   // Type inference
   export type IPermissionsCreateData = z.infer<typeof PermissionsCreateDataSchema>;
   ```
...
./bin/systemprompt dev generate-types permissions
```
...
1. **Service Methods**
   ...
   import { 
     PermissionsCreateDataSchema,
     PermissionsUpdateDataSchema,
     PermissionsRowSchema 
   } from '../types/permissions.module.generated';
   
   // Validate inputs
   async create(data: unknown): Promise<IPermissions> {
     const validated = PermissionsCreateDataSchema.parse(data);
     const result = await this.repository.create(validated);
     return PermissionsRowSchema.parse(result); // Validate output
   }
   ```
...
2. **Repository Methods**
   ...
   async findById(id: string): Promise<IPermissions | null> {
     const result = await this.db.query(...);
     return result ? PermissionsRowSchema.parse(result) : null;
   }
   ```
...
3. **CLI Commands**
   ...
   - See `/var/www/html/systemprompt-os/rules/src/modules/core/permissions/cli/rules.md`
...
4. **Module Exports**
   ...
   get exports(): IPermissionsModuleExports {
     ...
     return PermissionsModuleExportsSchema.parse(exports);
   }
   ```
...
import { 
  ...
  PermissionsRowSchema,
  type IPermissions,
  type IPermissionsCreateData,
  type IPermissionsUpdateData
} from '../types/permissions.module.generated';
...
export class PermissionsService {
  ...
  async create(data: unknown): Promise<IPermissions> {
    ...
    // Validate output
    return PermissionsRowSchema.parse(result);
  }
  
  async update(id: string, data: unknown): Promise<IPermissions> {
    // Validate partial update data
    const validated = PermissionsUpdateDataSchema.parse(data);
    
    const result = await this.repository.update(id, validated);
    return PermissionsRowSchema.parse(result);
  }
  
  async list(filters: unknown): Promise<IPermissions[]> {
    // Define and validate filter schema
    const filterSchema = z.object({
      status: PermissionsStatusSchema.optional(),
    ...
    return z.array(PermissionsRowSchema).parse(results);
  }
}
```
...
```typescript
// repositories/permissions.repository.ts
import { PermissionsRowSchema } from '../types/database.generated';

export class PermissionsRepository {
  async create(data: IPermissionsCreateData): Promise<IPermissions> {
    const result = await this.db.query(
      `INSERT INTO permissions (...) VALUES (...) RETURNING *`,
      [...]
    );
    
    // Validate database response
    return PermissionsRowSchema.parse(result.rows[0]);
  }
