...
### permissions.service.ts (MANDATORY)
...
export class PermissionsService {
  ...
  async createPermission(data: IPermissionCreateData): Promise<IPermission> {
    ...
    return await this.repository.createPermission(encryptedData);
  }
}
```
...
import {
  ...
  type IPermissionUpdateData
} from '@/modules/core/permissions/types/permissions.module.generated';
import type { IPermissionsService } from '@/modules/core/permissions/types/permissions.service.generated';
import { PermissionsStatus } from '@/modules/core/permissions/types/database.generated';
...
- **MUST** use `PermissionsRepository.getInstance()` for data access (main service only)
...
export class PermissionsService {
  ...
  async createPermission(data: IPermissionCreateData, userId: string): Promise<IPermission> {
    ...
    this.eventBus.on<UserDataRequestEvent>(
      ...
      async (event) => {
        try {
          ...
          this.logger?.error(LogSource.PERMISSIONS, 'Failed to handle user data request', { error });
        }
      }
    );
  }
...
export class PermissionsService {
  ...
  private constructor() {
    ...
    try {
      ...
      this.logger?.warn(LogSource.PERMISSIONS, 'Audit service not available');
    }
  }
...
async createPermission(data: IPermissionCreateData): Promise<IPermission> {
  ...
  await this.validatePermissionData(data);

  const id = randomUUID();
  const now = new Date();

  this.logger?.info(LogSource.PERMISSIONS, `Creating permission: ${data.name}`);

  const permission = await this.repository.createPermission({
    id,
    ...data,
    status: PermissionsStatus.ACTIVE,
    created_at: now.toISOString(),
    updated_at: now.toISOString()
  });

  // Emit events for other modules
  this.eventBus.emit(PermissionsEvents.{ENTITY}_CREATED, {
...
### Validation Methods
```typescript
private async validatePermissionData(data: IPermissionCreateData): Promise<void> {
  ...
  if (existing) {
    throw new Error(`Permission name already exists: ${data.name}`);
  }
...
```typescript
private setupEventHandlers(): void {
  this.eventBus.on<PermissionDataRequestEvent>(
    PermissionsEvents.{ENTITY}_DATA_REQUEST, 
    async (data: unknown) => {
      try {
        const event = data as PermissionDataRequestEvent;
        const permission = await this.getPermission(event.permissionId);
        
        const response: PermissionDataResponseEvent = {
          requestId: event.requestId,
          permission: permission ? { /* mapped data */ } : null
        };
        
        this.eventBus.emit(PermissionsEvents.{ENTITY}_DATA_RESPONSE, response);
      } catch (error) {
        this.logger?.error(LogSource.PERMISSIONS, 'Event handler error', { error });
...
### Return Types
- **MUST** return domain interfaces (e.g., `IPermission`), not database rows
...
if (data.status === 'inactive' && hasActiveRelations) {
  throw new Error('Cannot deactivate permission with active relations');
}
...
```typescript
// Operation start
this.logger?.info(LogSource.PERMISSIONS, `Starting operation: ${operation}`);

// Success
this.logger?.info(LogSource.PERMISSIONS, `Operation completed: ${operation}`);

// Errors
this.logger?.error(LogSource.PERMISSIONS, 'Operation failed', {
...
### Log Sources
- **MUST** use module-specific LogSource (e.g., `LogSource.PERMISSIONS`)
