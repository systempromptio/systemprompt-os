...
### system.service.ts (MANDATORY)
**MUST** be named exactly as the module name (e.g., `system.service.ts` for system module). This is the primary service that implements the main module interface.
**MUST** be located directly in the services directory (e.g., `/services/system.service.ts`), NOT inside a subfolder.
...
### Main Module Service
The primary service (e.g., `system.service.ts`) serves as:
...
```
services/
├── system.service.ts           # Main module service (MANDATORY - at root level)
├── validation.service.ts         # Additional service (at root level)
├── system/                     # Optional support folder for system.service.ts
│   ├── helpers.ts
│   ├── validators.ts
│   └── types.ts
...
export class SystemService {
  ...
  async createSystem(data: ISystemCreateData): Promise<ISystem> {
    ...
    return await this.repository.createSystem(encryptedData);
  }
}
```
...
} from '@/modules/core/system/types/system.module.generated';
import type { ISystemService } from '@/modules/core/system/types/system.service.generated';
import { SystemStatus } from '@/modules/core/system/types/database.generated';
...
- **MUST** implement the auto-generated service interface from `types/system.service.generated.ts` (main service only)
...
- **MUST** use `SystemRepository.getInstance()` for data access (main service only)
...
export class SystemService {
  ...
  async createSystem(data: ISystemCreateData, userId: string): Promise<ISystem> {
    ...
    this.eventBus.on<UserDataRequestEvent>(
      ...
      async (event) => {
        try {
          ...
          this.logger?.error(LogSource.SYSTEM, 'Failed to handle user data request', { error });
        }
      }
    );
  }
...
export class SystemService {
  ...
  private constructor() {
    ...
    try {
      ...
      this.logger?.warn(LogSource.SYSTEM, 'Audit service not available');
    }
  }
...
async createSystem(data: ISystemCreateData): Promise<ISystem> {
  ...
  this.logger?.info(LogSource.SYSTEM, `Creating system: ${data.name}`);
  ...
  this.eventBus.emit(SystemEvents.{ENTITY}_CREATED, {
    systemId: system.id,
    timestamp: now
  });
...
### Validation Methods
```typescript
private async validateSystemData(data: ISystemCreateData): Promise<void> {
  ...
  if (existing) {
    throw new Error(`System name already exists: ${data.name}`);
  }
...
```typescript
private setupEventHandlers(): void {
  this.eventBus.on<SystemDataRequestEvent>(
    SystemEvents.{ENTITY}_DATA_REQUEST, 
    async (data: unknown) => {
      try {
        const event = data as SystemDataRequestEvent;
        const system = await this.getSystem(event.systemId);
        
        const response: SystemDataResponseEvent = {
          requestId: event.requestId,
          system: system ? { /* mapped data */ } : null
        };
        
        this.eventBus.emit(SystemEvents.{ENTITY}_DATA_RESPONSE, response);
      } catch (error) {
        this.logger?.error(LogSource.SYSTEM, 'Event handler error', { error });
...
### Generated Types
- **MUST** use auto-generated interfaces from `types/system.module.generated.ts`
...
### Return Types
- **MUST** return domain interfaces (e.g., `ISystem`), not database rows
...
if (data.status === 'inactive' && hasActiveRelations) {
  throw new Error('Cannot deactivate system with active relations');
}
...
```typescript
// Operation start
this.logger?.info(LogSource.SYSTEM, `Starting operation: ${operation}`);

// Success
this.logger?.info(LogSource.SYSTEM, `Operation completed: ${operation}`);

// Errors
this.logger?.error(LogSource.SYSTEM, 'Operation failed', {
...
### Log Sources
- **MUST** use module-specific LogSource (e.g., `LogSource.SYSTEM`)
