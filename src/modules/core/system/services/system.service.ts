...
export class SystemService {
  ...
  async getSystemInfo(): Promise<ISystemInfo> {
    ...
    const moduleStats = {
      ...
      active: modules.filter(m => m.status === SystemModulesStatus.ACTIVE).length,
      inactive: modules.filter(m => m.status === SystemModulesStatus.INACTIVE).length,
      error: modules.filter(m => m.status === SystemModulesStatus.ERROR).length,
    };
  ...
  async checkHealth(): Promise<ISystemHealth> {
    ...
    try {
      const modules = await this.repository.getAllModules();
      const errorModules = modules.filter(m => m.status === SystemModulesStatus.ERROR);
