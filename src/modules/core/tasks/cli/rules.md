...
./bin/systemprompt tasks {command}
...
export const command: ICLICommand = {
  ...
  execute: async (context: ICLIContext): Promise<void> => {
    ...
    try {
      ...
      logger.error(LogSource.TASKS, 'Command failed', { error });
      process.exit(1);
    }
  },
};
...
2. **Command Implementation**: Each command is implemented in `src/modules/core/tasks/cli/{command}.ts`
3. **Database Storage**: During bootstrap, commands are registered in the `cli_commands` table with:
   - `command_path`: Full path like `tasks:command` (e.g., `tasks:create`)
   - `executor_path`: Path to the TypeScript file implementing the command
   - Command metadata (description, options, etc.)
4. **CLI Execution**: When running `systemprompt tasks {command}`:
   - The CLI main.ts loads commands from the database
   - Parses the command path (`tasks:{command}`) to find the executor
   - Dynamically imports the command module
   - Executes the command with parsed context

### Command Handler Resolution
- The handler string in `cli/index.ts` (e.g., `handler: 'tasks:{command}'`) maps to the command path
...
- `tasks.module.generated.ts` - Create/Update data schemas
- `tasks.service.generated.ts` - Service method schemas
...
```typescript
import { UserUpdateDataSchema } from '../types/tasks.module.generated';
...
```typescript
// In tasks/utils/cli-validation.ts
import { z } from 'zod';
import { CliOutputService } from '@/modules/core/cli/services/cli-output.service';
import { 
  UserCreateDataSchema,
...
3. **Type Generation**:
   - Run `./bin/systemprompt dev generate-types tasks` after schema changes
