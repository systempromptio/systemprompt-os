...
### tasks.service.ts (MANDATORY)
**MUST** be named exactly as the module name (e.g., `tasks.service.ts` for tasks module). This is the primary service that implements the main module interface.
**MUST** be located directly in the services directory (e.g., `/services/tasks.service.ts`), NOT inside a subfolder.
...
### Main Module Service
The primary service (e.g., `tasks.service.ts`) serves as:
...
```
services/
├── tasks.service.ts           # Main module service (MANDATORY - at root level)
├── validation.service.ts         # Additional service (at root level)
├── tasks/                     # Optional support folder for tasks.service.ts
│   ├── helpers.ts
│   ├── validators.ts
│   └── types.ts
...
export class TasksService {
  ...
  async createTask(data: ITaskCreateData): Promise<ITask> {
    ...
    return await this.repository.createTask(encryptedData);
  }
}
```
...
import { TasksRepository } from '@/modules/core/tasks/repositories/tasks.repository';
...
import { TasksStatus } from '@/modules/core/tasks/types/database.generated';
...
export class TasksService implements ITasksService {
  ...
  async initialize(): Promise<void> {
    ...
    this.logger?.info(LogSource.TASKS, 'TasksService initialized');
  }
...
- **MUST** implement the auto-generated service interface from `types/tasks.service.generated.ts` (main service only)
...
- **MUST** use `TasksRepository.getInstance()` for data access (main service only)
...
export class TasksService {
  ...
  async createTask(data: ITaskCreateData, userId: string): Promise<ITask> {
    ...
    this.eventBus.on<UserDataRequestEvent>(
      ...
      async (event) => {
        try {
          ...
          this.logger?.error(LogSource.TASKS, 'Failed to handle user data request', { error });
        }
      }
    );
  }
...
export class TasksService {
  ...
  private constructor() {
    ...
    try {
      ...
      this.logger?.warn(LogSource.TASKS, 'Audit service not available');
    }
  }
...
async createTask(data: ITaskCreateData): Promise<ITask> {
  ...
  this.logger?.info(LogSource.TASKS, `Creating task: ${data.name}`);
  ...
  this.eventBus.emit(TasksEvents.{ENTITY}_CREATED, {
    taskId: task.id,
    timestamp: now
  });
...
### Validation Methods
```typescript
private async validateTaskData(data: ITaskCreateData): Promise<void> {
  ...
  if (existing) {
    throw new Error(`Task name already exists: ${data.name}`);
  }
...
```typescript
private setupEventHandlers(): void {
  this.eventBus.on<TaskDataRequestEvent>(
    TasksEvents.{ENTITY}_DATA_REQUEST, 
    async (data: unknown) => {
      try {
        const event = data as TaskDataRequestEvent;
        const task = await this.getTask(event.taskId);
        
        const response: TaskDataResponseEvent = {
          requestId: event.requestId,
          task: task ? { /* mapped data */ } : null
        };
        
        this.eventBus.emit(TasksEvents.{ENTITY}_DATA_RESPONSE, response);
      } catch (error) {
        this.logger?.error(LogSource.TASKS, 'Event handler error', { error });
...
### Generated Types
- **MUST** use auto-generated interfaces from `types/tasks.module.generated.ts`
...
### Return Types
- **MUST** return domain interfaces (e.g., `ITask`), not database rows
...
if (data.status === 'inactive' && hasActiveRelations) {
  throw new Error('Cannot deactivate task with active relations');
}
...
```typescript
// Operation start
this.logger?.info(LogSource.TASKS, `Starting operation: ${operation}`);

// Success
this.logger?.info(LogSource.TASKS, `Operation completed: ${operation}`);

// Errors
this.logger?.error(LogSource.TASKS, 'Operation failed', {
...
### Log Sources
- **MUST** use module-specific LogSource (e.g., `LogSource.TASKS`)
