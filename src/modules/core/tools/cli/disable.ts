/**
 * @fileoverview Disable command for tools module
 * @module tools/cli/disable
 */
...
export const command = {
  execute: async (context: CLIContext): Promise<void> => {
    try {
      ...
      const success = await toolsModule.exports.disableTool(toolName);
      
      if (success) {
        console.log(`✓ Tool '${toolName}' has been disabled`);
      } else {
        console.error(`Failed to disable tool '${toolName}'`);
        process.exit(1);
      }
    } catch (error) {
