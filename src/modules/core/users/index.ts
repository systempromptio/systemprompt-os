...
export class UsersModule implements ModuleInterface {
  name = 'users';
  version = '1.0.0';
  ...
  async initialize(context: { config?: any; logger?: any }): Promise<void> {
    ...
    this.activityService = new ActivityService(activityRepo, this.config.activity || {}, this.logger);
  ...
  async healthCheck(): Promise<{ healthy: boolean; message?: string }> {
    try {
      ...
      return {
        healthy: false,
        message: `Users module health check failed: ${error}`
      };
    }
  }
