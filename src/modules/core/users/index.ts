...
export class UsersModule implements IModule<IUsersModuleExports> {
  ...
  async healthCheck(): Promise<{ healthy: boolean; message?: string }> {
    if (!this.initialized) {
      return { healthy: false, message: 'Users module not initialized' };
    }
    if (!this.started) {
      return { healthy: false, message: 'Users module not started' };
    }
    return { healthy: true, message: 'Users module is healthy' };
  }
