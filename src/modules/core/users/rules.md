...
import { UsersModuleExportsSchema } from './types/users.module.generated';

// Module MUST validate its structure
const moduleSchema = createModuleSchema(UsersModuleExportsSchema);
...
- `users.module.generated.ts` - Generated domain interfaces and Zod schemas from service methods
...
2. **Domain Types & Schemas**
   ...
   export const UsersCreateDataSchema = z.object({ ... });
   export const UsersUpdateDataSchema = z.object({ ... }).partial();
   
   // Type inference
   export type IUsersCreateData = z.infer<typeof UsersCreateDataSchema>;
   ```
...
./bin/systemprompt dev generate-types users
```
...
1. **Service Methods**
   ...
   import { 
     UsersCreateDataSchema,
     UsersUpdateDataSchema,
     UsersRowSchema 
   } from '../types/users.module.generated';
   
   // Validate inputs
   async create(data: unknown): Promise<IUsers> {
     const validated = UsersCreateDataSchema.parse(data);
     const result = await this.repository.create(validated);
     return UsersRowSchema.parse(result); // Validate output
   }
   ```
...
2. **Repository Methods**
   ...
   async findById(id: string): Promise<IUsers | null> {
     ...
     return result ? UsersRowSchema.parse(result) : null;
   }
   ```
...
3. **CLI Commands**
   ...
   - See `/var/www/html/systemprompt-os/rules/src/modules/core/users/cli/rules.md`
...
4. **Module Exports**
   ...
   get exports(): IUsersModuleExports {
     ...
     return UsersModuleExportsSchema.parse(exports);
   }
   ```
...
import { 
  ...
  UsersRowSchema,
  type IUsers,
  type IUsersCreateData,
  type IUsersUpdateData
} from '../types/users.module.generated';
...
export class UsersService {
  ...
  async create(data: unknown): Promise<IUsers> {
    ...
    // Validate output
    return UsersRowSchema.parse(result);
  }
  
  async update(id: string, data: unknown): Promise<IUsers> {
    // Validate partial update data
    const validated = UsersUpdateDataSchema.parse(data);
    
    const result = await this.repository.update(id, validated);
    return UsersRowSchema.parse(result);
  }
  
  async list(filters: unknown): Promise<IUsers[]> {
    ...
    return z.array(UsersRowSchema).parse(results);
  }
}
```
...
export class UsersRepository {
  async create(data: IUsersCreateData): Promise<IUsers> {
    const result = await this.db.query(
    ...
    return UsersRowSchema.parse(result.rows[0]);
  }
