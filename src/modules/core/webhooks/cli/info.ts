...
export const command = {
  execute: async (context: CLIContext): Promise<void> => {
    ...
    try {
      ...
      if (options.format === 'json') {
        ...
        try {
          ...
          if (stats.total_deliveries > 0) {
            console.log(`Success Rate:        ${((stats.successful_deliveries / stats.total_deliveries) * 100).toFixed(1)}%`);
      ...
      console.error('Failed to get webhook info:', error instanceof Error ? error.message : 'Unknown error');
      process.exit(1);
    }
  }
};