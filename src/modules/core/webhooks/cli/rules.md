...
./bin/systemprompt webhooks {command}
...
- **MUST** follow the exact pattern:
  ...
  export const webhooksCommands = {
    name: 'webhooks',
    alias: '{shortname}',  
    description: 'Webhooks module commands',
...
export const command: ICLICommand = {
  ...
  execute: async (context: ICLIContext): Promise<void> => {
    ...
    try {
      const service = WebhooksService.getInstance();
      ...
      logger.error(LogSource.WEBHOOKS, 'Command failed', { error });
      process.exit(1);
    }
  },
};
...
2. **Command Implementation**: Each command is implemented in `src/modules/core/webhooks/cli/{command}.ts`
3. **Database Storage**: During bootstrap, commands are registered in the `cli_commands` table with:
   - `command_path`: Full path like `webhooks:command` (e.g., `webhooks:create`)
   - `executor_path`: Path to the TypeScript file implementing the command
   - Command metadata (description, options, etc.)
4. **CLI Execution**: When running `systemprompt webhooks {command}`:
   - The CLI main.ts loads commands from the database
   - Parses the command path (`webhooks:{command}`) to find the executor
   - Dynamically imports the command module
   - Executes the command with parsed context

### Command Handler Resolution
- The handler string in `cli/index.ts` (e.g., `handler: 'webhooks:{command}'`) maps to the command path
...
- `webhooks.module.generated.ts` - Create/Update data schemas
- `webhooks.service.generated.ts` - Service method schemas
...
} from '../types/webhooks.module.generated';
...
```typescript
import { UserUpdateDataSchema } from '../types/webhooks.module.generated';
...
```typescript
// In webhooks/utils/cli-validation.ts
import { z } from 'zod';
import { CliOutputService } from '@/modules/core/cli/services/cli-output.service';
import { 
  UserCreateDataSchema,
...
3. **Type Generation**:
   - Run `./bin/systemprompt dev generate-types webhooks` after schema changes
