...
- `repositories/` - Data access layer
  - `webhooks.repository.ts` - Repository implementation
- `services/` - Business logic layer
  - `webhooks.service.ts` - Service implementation
...
### index.ts Structure
- Must extend `BaseModule<IWebhooksModuleExports>`
...
import { WebhooksModuleExportsSchema } from './types/webhooks.module.generated';

// Module MUST validate its structure
const moduleSchema = createModuleSchema(WebhooksModuleExportsSchema);
...
```typescript
export class WebhooksModule extends BaseModule<IWebhooksModuleExports> {
...
- Location: `/tests/integration/modules/core/webhooks/webhooks.integration.test.ts`
...
- `webhooks.module.generated.ts` - Generated domain interfaces and Zod schemas from service methods
- `webhooks.service.generated.ts` - Generated service validation schemas from TypeScript AST

### What Gets Generated:
1. **Database Types & Schemas**
   ```typescript
   // Automatically generated from schema.sql
   export interface IWebhooksRow { ... }
   export const WebhooksRowSchema = z.object({ ... });
   
   // Enums with validation
   export enum WebhooksStatus { ... }
   export const WebhooksStatusSchema = z.nativeEnum(WebhooksStatus);
   ```

2. **Domain Types & Schemas**
   ```typescript
   // Create/Update data schemas
   export const WebhooksCreateDataSchema = z.object({ ... });
   export const WebhooksUpdateDataSchema = z.object({ ... }).partial();
   
   // Type inference
   export type IWebhooksCreateData = z.infer<typeof WebhooksCreateDataSchema>;
   ```

3. **Module Export Schemas**
   ```typescript
   // Validates module's export interface
   export const WebhooksModuleExportsSchema = z.object({
...
./bin/systemprompt dev generate-types webhooks
```
...
1. **Service Methods**
   ...
   import { 
     WebhooksCreateDataSchema,
     WebhooksUpdateDataSchema,
     WebhooksRowSchema 
   } from '../types/webhooks.module.generated';
   
   // Validate inputs
   async create(data: unknown): Promise<IWebhooks> {
     const validated = WebhooksCreateDataSchema.parse(data);
     const result = await this.repository.create(validated);
     return WebhooksRowSchema.parse(result); // Validate output
   }
   ```
...
2. **Repository Methods**
   ...
   async findById(id: string): Promise<IWebhooks | null> {
     const result = await this.db.query(...);
     return result ? WebhooksRowSchema.parse(result) : null;
   }
   ```
...
3. **CLI Commands**
   ...
   - See `/var/www/html/systemprompt-os/rules/src/modules/core/webhooks/cli/rules.md`
...
4. **Module Exports**
   ...
   get exports(): IWebhooksModuleExports {
     ...
     return WebhooksModuleExportsSchema.parse(exports);
   }
   ```
...
import { 
  ...
  WebhooksRowSchema,
  type IWebhooks,
  type IWebhooksCreateData,
  type IWebhooksUpdateData
} from '../types/webhooks.module.generated';
...
export class WebhooksService {
  ...
  async create(data: unknown): Promise<IWebhooks> {
    ...
    // Validate output
    return WebhooksRowSchema.parse(result);
  }
  
  async update(id: string, data: unknown): Promise<IWebhooks> {
    // Validate partial update data
    const validated = WebhooksUpdateDataSchema.parse(data);
    
    const result = await this.repository.update(id, validated);
    return WebhooksRowSchema.parse(result);
  }
  
  async list(filters: unknown): Promise<IWebhooks[]> {
    // Define and validate filter schema
    const filterSchema = z.object({
      status: WebhooksStatusSchema.optional(),
    ...
    return z.array(WebhooksRowSchema).parse(results);
  }
}
```
...
```typescript
// repositories/webhooks.repository.ts
import { WebhooksRowSchema } from '../types/database.generated';

export class WebhooksRepository {
  async create(data: IWebhooksCreateData): Promise<IWebhooks> {
    const result = await this.db.query(
      `INSERT INTO webhooks (...) VALUES (...) RETURNING *`,
      [...]
    );
    
    // Validate database response
    return WebhooksRowSchema.parse(result.rows[0]);
  }
