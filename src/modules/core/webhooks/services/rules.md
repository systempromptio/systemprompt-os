...
### webhooks.service.ts (MANDATORY)
**MUST** be named exactly as the module name (e.g., `webhooks.service.ts` for webhooks module). This is the primary service that implements the main module interface.
**MUST** be located directly in the services directory (e.g., `/services/webhooks.service.ts`), NOT inside a subfolder.
...
### Main Module Service
The primary service (e.g., `webhooks.service.ts`) serves as:
...
```
services/
├── webhooks.service.ts           # Main module service (MANDATORY - at root level)
├── validation.service.ts         # Additional service (at root level)
├── webhooks/                     # Optional support folder for webhooks.service.ts
│   ├── helpers.ts
│   ├── validators.ts
│   └── types.ts
...
export class WebhooksService {
  ...
  private constructor() {
    this.repository = WebhooksRepository.getInstance();
    this.validationService = ValidationService.getInstance();
    this.encryptionService = EncryptionService.getInstance();
  }
  
  async createWebhook(data: IWebhookCreateData): Promise<IWebhook> {
    ...
    return await this.repository.createWebhook(encryptedData);
  }
}
```
...
import { WebhooksRepository } from '@/modules/core/webhooks/repositories/webhooks.repository';
import { EventBusService } from '@/modules/core/events/services/event-bus.service';
import {
  type IWebhook,
  type IWebhookCreateData,
  type IWebhookUpdateData
} from '@/modules/core/webhooks/types/webhooks.module.generated';
import type { IWebhooksService } from '@/modules/core/webhooks/types/webhooks.service.generated';
import { WebhooksStatus } from '@/modules/core/webhooks/types/database.generated';
...
export class WebhooksService implements IWebhooksService {
  ...
  async initialize(): Promise<void> {
    ...
    this.logger?.info(LogSource.WEBHOOKS, 'WebhooksService initialized');
  }
...
- **MUST** implement the auto-generated service interface from `types/webhooks.service.generated.ts` (main service only)
...
- **MUST** use `WebhooksRepository.getInstance()` for data access (main service only)
...
export class WebhooksService {
  ...
  async createWebhook(data: IWebhookCreateData, userId: string): Promise<IWebhook> {
    ...
    this.eventBus.on<UserDataRequestEvent>(
      ...
      async (event) => {
        try {
          ...
          this.logger?.error(LogSource.WEBHOOKS, 'Failed to handle user data request', { error });
        }
      }
    );
  }
...
export class WebhooksService {
  ...
  private constructor() {
    ...
    try {
      ...
      this.logger?.warn(LogSource.WEBHOOKS, 'Audit service not available');
    }
  }
...
async createWebhook(data: IWebhookCreateData): Promise<IWebhook> {
  ...
  await this.validateWebhookData(data);

  const id = randomUUID();
  const now = new Date();

  this.logger?.info(LogSource.WEBHOOKS, `Creating webhook: ${data.name}`);

  const webhook = await this.repository.createWebhook({
    id,
    ...data,
    status: WebhooksStatus.ACTIVE,
    created_at: now.toISOString(),
    updated_at: now.toISOString()
  });

  // Emit events for other modules
  this.eventBus.emit(WebhooksEvents.{ENTITY}_CREATED, {
...
### Validation Methods
```typescript
private async validateWebhookData(data: IWebhookCreateData): Promise<void> {
  ...
  if (existing) {
    throw new Error(`Webhook name already exists: ${data.name}`);
  }
...
```typescript
private setupEventHandlers(): void {
  this.eventBus.on<WebhookDataRequestEvent>(
    WebhooksEvents.{ENTITY}_DATA_REQUEST, 
    async (data: unknown) => {
      try {
        const event = data as WebhookDataRequestEvent;
        const webhook = await this.getWebhook(event.webhookId);
        
        const response: WebhookDataResponseEvent = {
          requestId: event.requestId,
          webhook: webhook ? { /* mapped data */ } : null
        };
        
        this.eventBus.emit(WebhooksEvents.{ENTITY}_DATA_RESPONSE, response);
      } catch (error) {
        this.logger?.error(LogSource.WEBHOOKS, 'Event handler error', { error });
...
### Generated Types
- **MUST** use auto-generated interfaces from `types/webhooks.module.generated.ts`
...
### Return Types
- **MUST** return domain interfaces (e.g., `IWebhook`), not database rows
...
if (data.status === 'inactive' && hasActiveRelations) {
  throw new Error('Cannot deactivate webhook with active relations');
}
...
```typescript
// Operation start
this.logger?.info(LogSource.WEBHOOKS, `Starting operation: ${operation}`);

// Success
this.logger?.info(LogSource.WEBHOOKS, `Operation completed: ${operation}`);

// Errors
this.logger?.error(LogSource.WEBHOOKS, 'Operation failed', {
...
### Log Sources
- **MUST** use module-specific LogSource (e.g., `LogSource.WEBHOOKS`)
