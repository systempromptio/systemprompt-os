...
export class WebhookDeliveryService implements IWebhookDeliveryService {
  ...
  async deliverOnce(webhook: Webhook, payload: WebhookPayload): Promise<WebhookDeliveryResult> {
    ...
    const deliveryKey = `${webhook.id}_${payload.event}_${Date.now().toString()}`;
    ...
    try {
      ...
      if (webhook.timeout !== null && webhook.timeout !== undefined) {
        setTimeout((): void => { controller.abort(); }, webhook.timeout);
      }
      ...
      if (!response.ok) {
        result.error = `HTTP ${response.status.toString()}: ${response.statusText}`;
      }
  ...
  async deliver(webhook: Webhook, payload: WebhookPayload): Promise<void> {
    if (webhook.status !== 'active') {
      this.logger.debug(LogSource.WEBHOOK, 'Skipping inactive webhook', {
        data: { webhookId: webhook.id }
      });
      return;
    }

    if (webhook.retry?.enabled === true) {
  ...
  async cancelDelivery(webhookId: string): Promise<void> {
    const keysToCancel = Array.from(this.activeDeliveries.keys()).filter(
      (key): boolean => { return key.startsWith(`${webhookId}_`) }
    );
  ...
  async cancelAllDeliveries(): Promise<void> {
    const { size: cancelledCount } = this.activeDeliveries;

    Array.from(this.activeDeliveries.values()).forEach((controller): void => {
      controller.abort();
    });
  ...
  private buildHeaders(webhook: Webhook, payload: WebhookPayload): Record<string, string> {
    ...
    if (webhook.auth != null) {
      const authHeaders = this.getAuthHeaders(webhook.auth);
  ...
  private getAuthHeaders(auth: WebhookAuth): Record<string, string> {
    ...
    switch (auth.type) {
      case 'bearer':
        if (auth.credentials.token != null && auth.credentials.token !== '') {
          authHeaders.Authorization = `Bearer ${auth.credentials.token}`;
        }
        break;
      case 'basic':
        if (auth.credentials.username != null && auth.credentials.username !== ''
            && auth.credentials.password != null && auth.credentials.password !== '') {
        ...
        break;
      case 'api_key':
        if (auth.credentials.api_key != null && auth.credentials.api_key !== '') {
          const { api_key } = auth.credentials;
   ...
   */
  private extractErrorMessage(error: unknown): { message: string } {
