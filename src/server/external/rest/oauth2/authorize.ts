...
import { z } from 'zod';
// Mock implementations to avoid dependency issues during development
...
const MockLogger: MockLoggerService = {
  getInstance: () => ({
    info: (source: string, message: string, metadata?: unknown) => {
      console.log(`[${source}] ${message}`, metadata || '');
    },
    error: (source: string, message: string, metadata?: unknown) => {
  ...
  exports: {
    providersService: () => ({
      getProviderInstance: (name: string) => ({
        name,
        getAuthorizationUrl: (state: string) => `/auth/${name}?state=${state}`,
        exchangeCodeForTokens: async (code: string) => ({ accessToken: code }),
        getUserInfo: async (token: string) => ({ id: 'user1', email: 'user@example.com', name: 'Test User', picture: 'avatar.jpg' })
      }),
      getAllProviderInstances: () => [
        { name: 'google', getAuthorizationUrl: () => '', exchangeCodeForTokens: async () => ({ accessToken: '' }), getUserInfo: async () => ({ id: '', email: '', name: '', picture: '' }) }
      ]
    }),
    authCodeService: () => ({
      createAuthorizationCode: async (params: IAuthorizationCodeData) => 'mock_auth_code',
      cleanupExpiredCodes: async () => {}
    }),
    oauthService: () => ({
      createOrUpdateUserFromOAuth: async (params: { provider: string; providerId: string; email: string; name?: string; avatarUrl?: string }) => 'user123'
    })
  }
});
import type {
...
const createAuthCodeParams = (
  ...
  const authCodeParams: IAuthorizationCodeData = {
    code: '', // Will be generated by the service
  ...
  const {
 provider, code_challenge, code_challenge_method
} = params;
...
export class AuthorizeEndpoint {
 ...
 code, state, error, errorDescription
      ...
      const oauthService = authModule.exports.oauthService();
