...
async function getProviderStatus(): Promise<ProviderStatus> {
  return {
    memory: process.env.MEMORYPROVIDER || 'not-configured',
    action: process.env.ACTIONPROVIDER || 'not-configured',
    auth: process.env.AUTHPROVIDER || 'not-configured',
    storage: process.env.STORAGEPROVIDER || 'not-configured'
  };
}
...
async function getLogsSummary(): Promise<LogsSummary> {
  const logsDir = join(CONFIG.STATEDIR || './state', 'logs');
...
/**
 * Status endpoint class for Express compatibility
 */
