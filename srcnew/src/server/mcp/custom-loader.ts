 ...
 */
const DEFAULT_OPTIONS: Partial<MCPLoaderOptions> = {
...
export class CustomMCPLoader {
  ...
  private async loadLocalServer(serverDir: string, customDir: string): Promise<void> {
    ...
    try {
      ...
      const serverName = module.CONFIG?.SERVER_NAME || serverDir;
      const serverVersion = module.CONFIG?.SERVER_VERSION || '0.0.0';
      const serverDescription = module.CONFIG?.SERVER_DESCRIPTION || `Custom MCP server: ${serverDir}`;
