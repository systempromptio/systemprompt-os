...
describe('CLI and Configuration Integration Test', () => {
  ...
  beforeEach(async () => {
    // Clear configuration data and ensure clean state
  ...
  describe('Configuration Management', () => {
    it('should manage system configuration settings using basic methods', async () => {
      ...
      for (const [key, value] of Object.entries(testConfig)) {
        ...
        // Immediately verify each set operation
        const setValue = await configService.get(key);
        expect(setValue).toEqual(value);
      }
      
      // List all configurations and verify they exist
      const configs = await configService.list();
      expect(configs.length).toBeGreaterThanOrEqual(4);
      
      // Verify all our test keys are present
    ...
    it('should support environment-specific configurations', async () => {
      ...
      for (const env of environments) {
        ...
        // Store environment-specific config (service handles serialization)
      ...
      expect((prodConfig as any).logLevel).toBe('error');
    });
  });
