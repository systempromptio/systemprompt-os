/**
 * @file Unit tests for test-module module.
 */

import { describe, it, expect, beforeEach, vi } from 'vitest';
import { TestModuleModule } from '@/modules/core/test-module/index';
import { TestModuleService } from '@/modules/core/test-module/services/test-module.service';
import { TestModuleRepository } from '@/modules/core/test-module/repositories/test-module.repository';
...
describe('TestModule Module', () => {
  let module: TestModuleModule;

  beforeEach(() => {
    // Clear singletons
    vi.clearAllMocks();
    (TestModuleService as any).instance = undefined;
    (TestModuleRepository as any).instance = undefined;
    
    module = new TestModuleModule();
  });
  ...
  describe('Module Lifecycle', () => {
    ...
    it('should throw error when starting without initialization', async () => {
      await expect(module.start()).rejects.toThrow('TestModule module not initialized');
    });
  ...
  describe('Module Exports', () => {
    it('should expose service through exports', async () => {
      ...
      expect(service).toBeInstanceOf(TestModuleService);
    });

    it('should throw error when accessing service before initialization', () => {
      const exports = module.exports;
      expect(() => exports.service()).toThrow('TestModule module not initialized');
    });
  });

  describe('TestModule Service', () => {
    let service: TestModuleService;

    beforeEach(async () => {
      service = TestModuleService.getInstance();
      await service.initialize();
    });

    it('should create a new test-module', async () => {
      const mockResult = {
        id: 'test-id',
      ...
      const repository = TestModuleRepository.getInstance();
    ...
    it('should get test-module by ID', async () => {
      const mockResult = {
        id: 'test-id',
        name: 'Test TestModule',
        createdAt: new Date(),
    ...
    it('should get all test-modules', async () => {
      const mockResults = [
        {
          id: 'test-1',
          name: 'Test 1',
    ...
    it('should update test-module', async () => {
      const existing = {
        id: 'test-id',
        name: 'Original Name',
        createdAt: new Date(),
    ...
    it('should throw error when updating non-existent test-module', async () => {
      const repository = TestModuleRepository.getInstance();
      vi.spyOn(repository, 'findById').mockResolvedValue(null);

      await expect(service.update('non-existent', { name: 'Test' }))
        .rejects.toThrow('TestModule not found');
    });

    it('should delete test-module', async () => {
      const existing = {
        id: 'test-id',
        name: 'Test',
        createdAt: new Date(),
    ...
    it('should throw error when deleting non-existent test-module', async () => {
      const repository = TestModuleRepository.getInstance();
      vi.spyOn(repository, 'findById').mockResolvedValue(null);
